...
export default function CashRegister({ data, reload, queue, onQueueConsumed, cart, setCart, pendingAction, onPendingConsumed }: { data: CashData; reload: () => Promise<void>; queue?: number[]; onQueueConsumed?: () => void; cart: CartItem[]; setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; pendingAction?: PendingCashAction | null; onPendingConsumed?: () => void }) {
  ...
  const [cartDiscountMode, setCartDiscountMode] = useState<"pct" | "eur">("pct");
  const [totalOverride, setTotalOverride] = useState("");
  const [payment, setPayment] = useState("cash");
  const [cashAmount, setCashAmount] = useState("");
  const [cashReceived, setCashReceived] = useState("");
  const [saleDone, setSaleDone] = useState<{ id: number; receiptNo: string; total: number; cash: number | null; change: number } | null>(null);
  ...
  const cartDiscountPct = !discountOpen ? 0 : cartDiscountMode === "eur"
    ? (subtotal > 0 ? Math.min(100, Math.max(0, (Number(cartDiscount) || 0) / subtotal * 100)) : 0)
    : Math.min(100, Math.max(0, Number(cartDiscount) || 0));
  const totalAfterCartDiscount = Math.round(subtotal * (1 - cartDiscountPct / 100) * 100) / 100;
  ...
  const createsResidualGift = hasReturn && total < -0.001;
  ...
  function addDraft(item: CartItem) {
    if (item.itemType === "return" && cart.some((row) => row.itemType === "return")) { setError("È già presente un reso in questa vendita (uno per scontrino)."); setModal(null); return; }
  ...
  return <Fragment key={item.key}>
    ...
    {details.length > 0 && <div className="cart-subitems"><div className="cart-subitems-head"><span>Dettaglio prodotti</span><span>Qtà</span><span>Costo</span></div>{details.map((detail, index) => <div className="cart-subitem" key={`${item.key}-detail-${index}`}><span>{detail.description}</span><span>{detail.quantity}</span><b>{money(detail.quantity * detail.unitPrice * (1 - detail.discountPercent / 100))}</b></div>)}<div className="cart-subitems-total"><span>Totale operazione</span><b>{money(Number(item.metadata.totalPrice) || details.reduce((sum, detail) => sum + detail.quantity * detail.unitPrice * (1 - detail.discountPercent / 100), 0))}</b></div></div>}
        ...
        <aside className="checkout">
          <div>{hasReturn && <div className="exchange-summary"><div><span>Valore reso</span><b>− {money(returnCredit)}</b></div><div><span>Nuovi articoli</span><b>{money(exchangePurchase)}</b></div><div className={total < 0 ? "refund" : "difference"}><span>{total > 0 ? "Differenza da incassare" : total < 0 ? createsResidualGift ? "Credito su nuovo buono" : "Rimborso al cliente" : "Cambio alla pari"}</span><strong>{money(Math.abs(total))}</strong></div></div>}<label className="discount-check"><input type="checkbox" checked={discountOpen} onChange={(event) => { const on = event.target.checked; setDiscountOpen(on); if (!on) setCartDiscount(""); setTotalOverride(""); }} /> Applica sconto sul totale</label>{discountOpen && <div className="disc-field cart-disc"><input type="number" min="0" step="0.01" value={cartDiscount} placeholder="0" onChange={(event) => { setCartDiscount(event.target.value); setTotalOverride(""); }} aria-label="Sconto sul totale" autoFocus /><button type="button" className="disc-toggle" onClick={() => setCartDiscountMode((m) => m === "pct" ? "eur" : "pct")} aria-label="Cambia unità sconto">{cartDiscountMode === "pct" ? "%" : "€"}</button></div>}{hasReturn ? <div className="grand-total"><span>Differenza finale</span><strong>{money(total)}</strong></div> : <><div className="grand-total"><span>TOTALE</span><strong>{money(subtotal)}</strong></div>{cartDiscountPct > 0 && <div className="grand-total discounted"><span>TOTALE SCONTATO</span><strong>{money(total)}</strong></div>}</>}</div>
          <div><p className="eyebrow">PAGAMENTO</p>{createsResidualGift && <div className="gift-credit-notice"><strong>Buono residuo automatico</strong><span>{hasGiftOriginReturn ? "Il buono originale verrà stornato. Il credito sarà trasferito su un nuovo buono intestato alla stessa persona." : "Il credito del reso sarà consegnato come buono regalo (nessun rimborso in contanti)."}</span></div>}<div className="payment-tabs">{[["cash", "Contanti"], ["card", "Carta"], ["mixed", "Misto"], ["gift", "Buono"], ...(data.user.role === "admin" ? [["bank", "Bonifico"]] : [])].map(([value, label]) => <button key={value} className={payment === value ? "active" : ""} onClick={() => setPayment(value)} disabled={createsResidualGift}>{label}</button>)}</div>{!createsResidualGift && payment === "mixed" && <div className="split-fields"><ClearableInput label="Contanti" type="number" step="0.01" value={cashAmount} onChange={setCashAmount} /><ClearableInput label="Carta" type="number" step="0.01" value={cardAmount} onChange={setCardAmount} /></div>}{!createsResidualGift && payment === "gift" && <div className="stack compact-stack"><ClearableInput label="Codice EAN buono" value={giftCode} onChange={setGiftCode} /><ClearableInput label="Importo da scalare" type="number" step="0.01" value={giftAmount} onChange={setGiftAmount} /><ClearableInput label="Residuo contanti" type="number" step="0.01" value={cashAmount} onChange={setCashAmount} /><ClearableInput label="Residuo carta" type="number" step="0.01" value={cardAmount} onChange={setCardAmount} /></div>}{!createsResidualGift && payment === "bank" && <div className="bank-payment-box"><label className="field"><span>Documento collegato al bonifico</span><select value={fiscalDocumentType} onChange={(event) => setFiscalDocumentType(event.target.value as "receipt" | "invoice")}><option value="receipt">Scontrino fiscale automatico</option><option value="invoice">Fattura automatica</option></select></label>{fiscalDocumentType === "invoice" && !customer && <div className="alert danger">Associa prima un cliente o un’azienda per intestare la fattura.</div>}<p className="muted">Importo bonifico: <strong>{money(total)}</strong></p></div>}{!createsResidualGift && !hasReturn && payment === "cash" && <div className="cash-received"><ClearableInput label="Contanti ricevuti dal cliente" type="number" step="0.01" value={cashReceived} onChange={setCashReceived} placeholder={total.toFixed(2)} inputMode="decimal" />{(Number(cashReceived) || 0) > 0 && <div className={`cash-change-live ${(Number(cashReceived) || 0) < total ? "short" : ""}`}><span>{(Number(cashReceived) || 0) < total ? "Mancano" : "Resto"}</span><strong>{money(Math.abs((Number(cashReceived) || 0) - total))}</strong></div>}</div>}</div>
          <button className={`primary checkout-button ${completing ? "is-loading" : ""}`} disabled={completing || !cart.length || (!createsResidualGift && payment === "bank" && fiscalDocumentType === "invoice" && !customer)} onClick={completeSale}>{completing ? <><span className="btn-spinner" aria-hidden="true" />Registrazione in corso…</> : hasReturn ? total < 0 ? createsResidualGift ? `Registra reso · genera buono ${money(Math.abs(total))}` : `Registra reso · rimborso ${money(Math.abs(total))}` : `Completa cambio · ${money(total)}` : `Completa vendita · ${money(total)}`}</button>
