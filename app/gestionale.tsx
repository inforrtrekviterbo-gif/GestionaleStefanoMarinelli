...
export default function Gestionale() {
  ...
  const navButton = ([id, icon, label]: readonly [string, string, string]) => <button key={id} className={page === id ? "active" : ""} onClick={() => { setPage(id); setMenuOpen(false); }}><MaterialIcon className="nav-icon">{icon}</MaterialIcon>{label}{id === "cash" && cart.length > 0 && <span className="nav-badge" title={`${cart.length} nel carrello`}>{cart.length}</span>}</button>;
