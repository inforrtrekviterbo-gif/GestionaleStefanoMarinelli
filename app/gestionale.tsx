...
function ScanInfo({ data, product, onAdd }: { data: Bootstrap; product: Product; onAdd: (product: Product) => void }) {
  ...
  return <div className="scan-card"><div className="scan-hero"><div className="scan-photo" style={photoKey ? { backgroundImage: `url(/api/products?key=${encodeURIComponent(photoKey)})` } : undefined}>{!photoKey && <MaterialIcon>inventory_2</MaterialIcon>}</div><div className="scan-hero-info"><p className="eyebrow">{product.brand} · {product.category}</p><h2 className="scan-title">{product.name}</h2><div className="scan-price">{money(product.price)}</div></div></div><div className="scan-variants">{group.map((item) => { const a = avail(item); const out = a.vt <= 0 && a.gs <= 0; const isMatch = item.id === product.id; return <div key={item.id} className={`scan-variant ${isMatch ? "match" : ""} ${out ? "out" : ""}`}><div className="scan-variant-main"><strong>{item.color} · {item.size}{isMatch && <span className="scan-badge">scansionato</span>}</strong><small>{item.sku}</small></div><div className="scan-variant-stock"><span className="chip">VT {a.vt}</span><span className="chip">GS {a.gs}</span></div><button className="primary small" disabled={out} onClick={() => onAdd(item)}>{out ? "Esaurito" : "Aggiungi"}</button></div>; })}</div></div>;
}
