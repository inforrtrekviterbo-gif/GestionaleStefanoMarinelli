...
function Transfers({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  const [fromStore, setFromStore] = useState<Store>(data.user.store ?? "Viterbo"); const [items, setItems] = useState<CartItem[]>([]); const [sender, setSender] = useState(""); const [receiver, setReceiver] = useState(""); const [carrier, setCarrier] = useState(""); const [reason, setReason] = useState("Trasferimento merce"); const [error, setError] = useState(""); const [last, setLast] = useState<{ id: number; code: string } | null>(null); const [editing, setEditing] = useState<{ transfer: Transfer; items: TransferAdminItem[] } | null>(null);
  const isAdmin = data.user.role === "admin";
...
function ProductDetail({ data, reload, groupKey, onBack }: { data: Bootstrap; reload: () => Promise<void>; groupKey: string; onBack: () => void }) {
  ...
  async function remove(item: Product) { if (!confirm(`Cancellare la variante ${item.color} ${item.size}?`)) return; setError(""); try { await post("deleteProduct", { id: item.id }); const remaining = data.products.filter((row) => (row.variantGroup || `single-${row.id}`) === groupKey && row.id !== item.id); await reload(); if (!remaining.length) onBack(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
  async function saveStock(item: Product, store: Store, value: string) { const quantity = Math.max(0, Math.round(Number(value) || 0)); if (quantity === (store === "Viterbo" ? item.viterboQty : item.granSassoQty)) return; setError(""); try { await post("setStock", { id: item.id, store, quantity }); await reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
  if (!base) return <section className="screen"><div className="screen-head"><div><button className="text-button" onClick={onBack}><MaterialIcon>arrow_back</MaterialIcon> Magazzino</button><h1>Prodotto non disponibile</h1></div></div><Empty>Questo prodotto non esiste più.</Empty></section>;
  return <section className="screen"><div className="screen-head"><div><button className="text-button" onClick={onBack}><MaterialIcon>arrow_back</MaterialIcon> Magazzino</button><p className="eyebrow">{base.category}</p><h1>{base.brand} · {base.name}</h1></div>{isAdmin && base.variantGroup && <button className="primary" onClick={() => setVariantSource({ product: base, mode: "variant" })}><MaterialIcon>add</MaterialIcon> Aggiungi variante</button>}</div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<div className="product-detail-top"><div className="panel product-detail-card">{photoKey && <img className="product-detail-photo" src={`/api/products?key=${encodeURIComponent(photoKey)}`} alt={base.name} />}<div className="metric-grid"><Metric label="Prezzo di vendita" value={money(base.price)} /><Metric label="Varianti" value={String(items.length)} /><Metric label="Giacenza Viterbo" value={String(totalViterbo)} /><Metric label="Giacenza Gran Sasso" value={String(totalGranSasso)} /></div></div></div><div className="panel"><div className="panel-title"><h2>Varianti</h2></div><div className="table-wrap"><table><thead><tr><th>Colore · Taglia</th><th>SKU / EAN</th><th>Prezzo</th><th>Viterbo</th><th>Gran Sasso</th>{isAdmin && <th>Azioni</th>}</tr></thead><tbody>{items.map((item) => <tr key={item.id} className={needsReorder(item) ? "warehouse-low" : ""}><td><strong>{item.color} · {item.size}</strong>{needsReorder(item) && <small className="status warning">da riordinare</small>}</td><td>{item.sku}<small>{item.eans.split(",").join(" · ")}</small></td><td>{money(item.price)}</td><td>{isAdmin ? <input key={`vt-${item.id}-${item.viterboQty}`} className="stock-input" type="number" min={item.viterboReserved} defaultValue={item.viterboQty} onBlur={(event) => void saveStock(item, "Viterbo", event.target.value)} /> : item.viterboQty}{item.viterboReserved > 0 && <small>{item.viterboReserved} pren.</small>}</td><td>{isAdmin ? <input key={`gs-${item.id}-${item.granSassoQty}`} className="stock-input" type="number" min={item.granSassoReserved} defaultValue={item.granSassoQty} onBlur={(event) => void saveStock(item, "Gran Sasso", event.target.value)} /> : item.granSassoQty}{item.granSassoReserved > 0 && <small>{item.granSassoReserved} pren.</small>}</td>{isAdmin && <td><div className="table-actions"><button className="secondary small" onClick={() => setEditing(item)}>Modifica</button><button className="secondary small" onClick={() => setVariantSource({ product: item, mode: "duplicate" })}>Duplica</button><button className="text-button negative" onClick={() => void remove(item)}>Rimuovi</button></div></td>}</tr>)}</tbody></table></div></div>{editing && <Modal title="Modifica variante" onClose={() => setEditing(null)}><ProductAdminForm product={editing} reload={reload} close={() => setEditing(null)} /></Modal>}{variantSource && <Modal title={variantSource.mode === "duplicate" ? "Duplica come variante" : "Aggiungi variante"} onClose={() => setVariantSource(null)}><VariantQuickForm product={variantSource.product} mode={variantSource.mode} reload={reload} close={() => setVariantSource(null)} /></Modal>}</section>;
}

function Warehouse({ data, reload, onOpenProduct }: { data: Bootstrap; reload: () => Promise<void>; onOpenProduct: (groupKey: string) => void }) {
  const [query, setQuery] = useState("");
  ...
  const normalized = query.trim().toLocaleLowerCase("it");
  const needsReorder = (item: Product) => (item.viterboQty - item.viterboReserved) <= item.viterboReorderLevel || (item.granSassoQty - item.granSassoReserved) <= item.granSassoReorderLevel;
  const filtered = data.products.filter((item) => (!normalized || searchableProduct(item).includes(normalized)) && (!onlyReorder || needsReorder(item)));
  const groups = new Map<string, { key: string; brand: string; name: string; category: string; price: number; variantGroup: string | null; items: Product[] }>();
  for (const item of filtered) {
    const key = item.variantGroup || `single-${item.id}`;
    const group = groups.get(key) ?? { key, brand: item.brand, name: item.name, category: item.category, price: item.price, variantGroup: item.variantGroup, items: [] as Product[] };
    group.items.push(item); groups.set(key, group);
  }
  const groupList = [...groups.values()].sort((a, b) => `${a.brand} ${a.name}`.localeCompare(`${b.brand} ${b.name}`, "it"));
  async function remove(id: number, label: string) { if (!confirm(`Cancellare ${label}?`)) return; setError(""); setMessage(""); try { await post("deleteProduct", { id }); setMessage("Variante rimossa."); await reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
  async function saveStock(item: Product, store: Store, value: string) {
    const quantity = Math.max(0, Math.round(Number(value) || 0));
    if (quantity === (store === "Viterbo" ? item.viterboQty : item.granSassoQty)) return;
    setError(""); try { await post("setStock", { id: item.id, store, quantity }); await reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
  }
  return <section className="screen"><div className="screen-head"><div><p className="eyebrow">GESTIONE MAGAZZINO</p><h1>Magazzino</h1><p className="muted inventory-intro">Prodotti, varianti e giacenze in un unico posto.</p></div>{isAdmin && <button className="primary" onClick={() => setShowNew(true)}><MaterialIcon>add_box</MaterialIcon> Nuovo prodotto</button>}</div>{message && <div className="alert success">{message}</div>}{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<EanLoadBar data={data} reload={reload} /><div className="panel"><div className="warehouse-toolbar"><ClearableInput label="Ricerca prodotto" value={query} onChange={setQuery} placeholder="Nome, marca, colore, SKU, EAN…" /><label className="checkbox-inline"><input type="checkbox" checked={onlyReorder} onChange={(event) => setOnlyReorder(event.target.checked)} /> Solo da riordinare</label></div><div className="table-wrap"><table><thead><tr><th>Prodotto / Variante</th><th>SKU / EAN</th><th>Prezzo</th><th>Viterbo</th><th>Gran Sasso</th><th>Azioni</th></tr></thead><tbody>{groupList.map((group) => <Fragment key={group.key}><tr className="warehouse-group-row"><td colSpan={6}><button type="button" className="text-button" title="Espandi/comprimi" onClick={() => setCollapsed((current) => ({ ...current, [group.key]: !current[group.key] }))}><MaterialIcon>{collapsed[group.key] ? "chevron_right" : "expand_more"}</MaterialIcon></button><button type="button" className="text-button warehouse-group-name" onClick={() => onOpenProduct(group.key)} title="Apri scheda prodotto"><strong>{group.brand} · {group.name}</strong> <small>{group.category} · {money(group.price)} · {group.items.length} {group.items.length === 1 ? "variante" : "varianti"}</small></button>{isAdmin && group.variantGroup && <button className="secondary small" onClick={() => setVariantSource({ product: group.items[0], mode: "variant" })}>+ Variante</button>}<button className="secondary small" onClick={() => onOpenProduct(group.key)}>Apri scheda</button></td></tr>{!collapsed[group.key] && group.items.map((item) => <tr key={item.id} className={needsReorder(item) ? "warehouse-low" : ""}><td><strong>{item.color} · {item.size}</strong>{needsReorder(item) && <small className="status warning">da riordinare</small>}</td><td>{item.sku}<small>{item.eans.split(",").join(" · ")}</small></td><td>{money(item.price)}</td><td>{isAdmin ? <input key={`vt-${item.id}-${item.viterboQty}`} className="stock-input" type="number" min={item.viterboReserved} defaultValue={item.viterboQty} onBlur={(event) => void saveStock(item, "Viterbo", event.target.value)} /> : item.viterboQty}{item.viterboReserved > 0 && <small>{item.viterboReserved} pren.</small>}</td><td>{isAdmin ? <input key={`gs-${item.id}-${item.granSassoQty}`} className="stock-input" type="number" min={item.granSassoReserved} defaultValue={item.granSassoQty} onBlur={(event) => void saveStock(item, "Gran Sasso", event.target.value)} /> : item.granSassoQty}{item.granSassoReserved > 0 && <small>{item.granSassoReserved} pren.</small>}</td><td><div className="table-actions">{isAdmin && <><button className="secondary small" onClick={() => setEditing(item)}>Modifica</button><button className="secondary small" onClick={() => setVariantSource({ product: item, mode: "duplicate" })}>Duplica</button><button className="text-button negative" onClick={() => void remove(item.id, `${item.name} ${item.color} ${item.size}`)}>Rimuovi</button></>}</div></td></tr>)}</Fragment>)}</tbody></table></div>{!groupList.length && <Empty>Nessun prodotto trovato.</Empty>}</div>{showNew && <Modal title="Nuovo prodotto" onClose={() => setShowNew(false)}><NewProductForm data={data} reload={reload} onCreated={() => setShowNew(false)} /></Modal>}{editing && <Modal title="Modifica variante" onClose={() => setEditing(null)}><ProductAdminForm product={editing} reload={reload} close={() => setEditing(null)} /></Modal>}{variantSource && <Modal title={variantSource.mode === "duplicate" ? "Duplica come variante" : "Aggiungi variante"} onClose={() => setVariantSource(null)}><VariantQuickForm product={variantSource.product} mode={variantSource.mode} reload={reload} close={() => setVariantSource(null)} /></Modal>}</section>;
}
...
const adminMenuGroups: { label: string; items: readonly (readonly [string, string, string])[] }[] = [
  ...
  { label: "Magazzino", items: [["warehouse", "inventory_2", "Magazzino"], ["transfers", "swap_horiz", "Trasferimenti"]] },
  ...
  { label: "Amministrazione", items: [["documents", "receipt_long", "Fatturazione"], ["analytics", "trending_up", "Analisi vendite"], ["fiscal", "print", "Registratori RT"], ["settings", "settings", "Impostazioni"]] },
];
...
export default function Gestionale() {
  const [user, setUser] = useState<User | null>(null); const [data, setData] = useState<Bootstrap | null>(null); const [loading, setLoading] = useState(true); const [page, setPage] = useState("dashboard"); const [productView, setProductView] = useState<string | null>(null); const [menuOpen, setMenuOpen] = useState(false); const [fatal, setFatal] = useState(""); const [syncState, setSyncState] = useState<"online" | "syncing" | "offline">("syncing");
  const openProduct = useCallback((groupKey: string) => { setProductView(groupKey); setPage("product"); }, []);
  ...
  const navButton = ([id, icon, label]: readonly [string, string, string]) => <button key={id} className={page === id ? "active" : ""} onClick={() => { setPage(id); setProductView(null); setMenuOpen(false); }}><MaterialIcon className="nav-icon">{icon}</MaterialIcon>{label}</button>;
  const content: Record<string, ReactNode> = { dashboard: <Dashboard data={data} />, cash: <NewCashRegister data={data} reload={reload} />, customers: <Customers data={data} reload={reload} />, warehouse: <Warehouse data={data} reload={reload} onOpenProduct={openProduct} />, product: productView ? <ProductDetail data={data} reload={reload} groupKey={productView} onBack={() => { setProductView(null); setPage("warehouse"); }} /> : <Warehouse data={data} reload={reload} onOpenProduct={openProduct} />, inventory: <Inventory data={data} />, reorder: <ReorderList data={data} />, transfers: <Transfers data={data} reload={reload} />, load: <QuickLoad data={data} reload={reload} />, documents: <Documents data={data} reload={reload} />, register: <Register data={data} />, reports: <CashReports data={data} />, gifts: <GiftSummary data={data} reload={reload} />, reservations: <ReservationSummary data={data} reload={reload} />, analytics: <Analytics data={data} />, fiscal: <FiscalRegisters data={data} reload={reload} />, settings: <Settings reload={reload} /> };
