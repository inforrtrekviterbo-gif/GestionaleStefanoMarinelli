...
type Reservation = { id: number; code: string; store: Store; customerId: number | null; productId: number | null; description: string; kind: string; totalPrice: number; depositAmount: number; balanceDue: number; status: string; issuedSaleId: number; createdAt: string; customerName: string; itemCount: number; itemDescriptions: string; expectedDelivery: string | null; depositPaid: number };
...
type Bootstrap = { user: User; products: Product[]; customers: Customer[]; sales: Sale[]; gifts: Gift[]; reservations: Reservation[]; transfers: Transfer[]; documents: BusinessDocument[]; saleItems?: SaleItemSummary[]; fiscalDevices: FiscalDevice[]; fiscalJobs: FiscalJob[]; generatedAt: string; services: { id: number; name: string; price: number; active: number }[]; reorderSettings?: { globalMinEnabled: boolean; globalMin: number; suggestTransfers: boolean; transferSafety: number } };
...
function EmptyGuide({ icon, title, text, children }: { icon: string; title: string; text: string; children?: ReactNode }) {
  return <div className="empty-guide"><MaterialIcon>{icon}</MaterialIcon><h3>{title}</h3><p>{text}</p>{children && <div className="empty-guide-actions">{children}</div>}</div>;
}

function ProductThumb({ photoKey, name, className = "" }: { photoKey: string | null | undefined; name: string; className?: string }) {
  const [failed, setFailed] = useState(false);
  if (photoKey && !failed) return <img className={className} src={`/api/products?key=${encodeURIComponent(photoKey)}`} alt={name} loading="lazy" onError={() => setFailed(true)} />;
...
function CustomerForm({ store, onSaved, onClose, customer = null }: { store: Store; onSaved: () => Promise<void>; onClose: () => void; customer?: Customer | null }) {
  const [form, setForm] = useState({ firstName: customer?.firstName ?? "", lastName: customer?.lastName ?? "", phone: customer?.phone ?? "", email: customer?.email ?? "", address: customer?.address ?? "", postalCode: customer?.postalCode ?? "", city: customer?.city ?? "", province: customer?.province ?? "", taxCode: customer?.taxCode ?? "" });
  const [error, setError] = useState("");
...
function Customers({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  type SortKey = "lastName" | "firstName" | "phone" | "email" | "city";
  const [query, setQuery] = useState(""); const [sortKey, setSortKey] = useState<SortKey>("lastName"); const [sortDir, setSortDir] = useState<"asc" | "desc">("asc");
  const [selected, setSelected] = useState<Customer | null>(null); const [history, setHistory] = useState<{ sales: Sale[]; items: { saleId: number; description: string; quantity: number; lineTotal: number }[] } | null>(null);
  const [newOpen, setNewOpen] = useState(false); const [editing, setEditing] = useState(false); const [showImport, setShowImport] = useState(false); const [error, setError] = useState("");
  const isAdmin = data.user.role === "admin";
  const filtered = useMemo(() => { const q = query.trim().toLocaleLowerCase("it"); return data.customers.filter((c) => !q || `${c.lastName} ${c.firstName} ${c.phone} ${c.email} ${c.city} ${c.taxCode}`.toLocaleLowerCase("it").includes(q)); }, [data.customers, query]);
  const sorted = useMemo(() => [...filtered].sort((a, b) => { const r = String(a[sortKey] ?? "").localeCompare(String(b[sortKey] ?? ""), "it", { sensitivity: "base" }); return sortDir === "asc" ? r : -r; }), [filtered, sortKey, sortDir]);
  function toggleSort(key: SortKey) { if (sortKey === key) setSortDir((d) => d === "asc" ? "desc" : "asc"); else { setSortKey(key); setSortDir("asc"); } }
  async function openCustomer(c: Customer) { setSelected(c); setHistory(null); setError(""); try { const r = await readJson(await fetch(`/api/data?view=customer&id=${c.id}`, { cache: "no-store" })); setHistory(r); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
  ...
  function printClients() {
    ...
    if (!popup) { alert("Consenti i popup del browser per stampare."); return; }
    popup.document.write(`<!doctype html><html lang="it"><head><meta charset="utf-8"><title>Clienti</title><style>body{font:14px Arial,sans-serif;color:#111;margin:28px}header{display:flex;align-items:center;gap:16px;border-bottom:3px solid #12bfe9;padding-bottom:14px;margin-bottom:18px}header img{height:54px}h1{margin:0 0 4px;font-size:22px}header p{color:#555;margin:0;font-size:12px}table{width:100%;border-collapse:collapse}th,td{padding:8px 7px;border-bottom:1px solid #bbb;text-align:left}th{font-size:11px;text-transform:uppercase}@media print{body{margin:12mm}}</style></head><body><header><img src="${window.location.origin}/ms-logo.png" alt="" /><div><h1>Clienti</h1><p>${safe(new Date().toLocaleString("it-IT"))} · ${sorted.length} clienti</p></div></header><table><thead><tr><th>Cognome</th><th>Nome</th><th>Telefono</th><th>Email</th><th>Comune</th></tr></thead><tbody>${sorted.map((c) => `<tr><td>${safe(c.lastName)}</td><td>${safe(c.firstName)}</td><td>${safe(c.phone)}</td><td>${safe(c.email)}</td><td>${safe(c.city)}</td></tr>`).join("")}</tbody></table><script>window.onload=()=>{const done=()=>{window.print();window.close()};const img=document.images[0];if(img&&!img.complete){img.onload=done;img.onerror=done;setTimeout(done,1500)}else done()}<\/script></body></html>`);
    popup.document.close();
  }
  function exportCsv() {
    const esc = (v: unknown) => { const str = String(v ?? ""); return /[",;\n]/.test(str) ? `"${str.replace(/"/g, '""')}"` : str; };
    const head = "cognome,nome,telefono,email,indirizzo,cap,comune,provincia,codice_fiscale";
    const lines = data.customers.map((c) => [c.lastName, c.firstName, c.phone, c.email, c.address, c.postalCode, c.city, c.province, c.taxCode].map(esc).join(","));
    const a = document.createElement("a"); a.href = `data:text/csv;charset=utf-8,${encodeURIComponent([head, ...lines].join("\n"))}`; a.download = "clienti.csv"; a.click();
  }
  const contact = selected ? [selected.address, selected.postalCode, selected.city, selected.province].filter(Boolean).join(" · ") : "";
  const Th = ({ k, label }: { k: SortKey; label: string }) => <th className="sortable" onClick={() => toggleSort(k)}>{label}{sortKey === k && <MaterialIcon>{sortDir === "asc" ? "arrow_upward" : "arrow_downward"}</MaterialIcon>}</th>;
  return <section className="screen"><div className="screen-head"><div><p className="eyebrow">ANAGRAFICHE E STORICO</p><h1>Clienti</h1><p className="muted inventory-intro">Per vendite importanti e garanzia: collega lo scontrino al cliente.</p></div><div className="head-controls"><button className="secondary small" onClick={printClients} disabled={!sorted.length}><MaterialIcon>print</MaterialIcon> Stampa</button><button className="secondary small" onClick={exportCsv} disabled={!data.customers.length}><MaterialIcon>download</MaterialIcon> Esporta CSV</button>{isAdmin && <button className="secondary small" onClick={() => setShowImport(true)}><MaterialIcon>upload_file</MaterialIcon> Importa CSV</button>}<button className="primary" onClick={() => setNewOpen(true)}><MaterialIcon>person_add</MaterialIcon> Nuovo cliente</button></div></div><div className="panel"><div className="search-hero"><MaterialIcon>search</MaterialIcon><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cerca cliente per cognome, nome, telefono, email…" /></div>{sorted.length ? <div className="table-wrap"><table className="clients-table"><thead><tr><Th k="lastName" label="Cognome" /><Th k="firstName" label="Nome" /><Th k="phone" label="Telefono" /><Th k="email" label="Email" /><Th k="city" label="Comune" /><th /></tr></thead><tbody>{sorted.map((c) => <tr key={c.id} className="clickable-row" onClick={() => void openCustomer(c)}><td><strong>{c.lastName || "—"}</strong></td><td>{c.firstName || "—"}</td><td>{c.phone || "—"}</td><td>{c.email || "—"}</td><td>{c.city || "—"}</td><td className="row-arrow"><MaterialIcon>chevron_right</MaterialIcon></td></tr>)}</tbody></table></div> : (data.customers.length === 0 ? <EmptyGuide icon="group" title="Nessun cliente" text="Aggiungi il primo cliente: bastano nome e cognome.">{<button className="primary" onClick={() => setNewOpen(true)}><MaterialIcon>person_add</MaterialIcon> Nuovo cliente</button>}</EmptyGuide> : <Empty>Nessun cliente trovato.</Empty>)}</div>{selected && <Modal title="Scheda cliente" wide guard={false} onClose={() => { setSelected(null); setHistory(null); }}><div className="customer-detail"><div className="panel-title"><h2>{customerLabel(selected)}</h2><div className="table-actions">{isAdmin && <><button className="secondary small" onClick={() => setEditing(true)}><MaterialIcon>edit</MaterialIcon> Modifica cliente</button><button className="text-button negative" onClick={() => void removeCustomer()}>Rimuovi</button></>}</div></div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<div className="customer-contacts">{selected.phone && <span><MaterialIcon>call</MaterialIcon>{selected.phone}</span>}{selected.email && <span><MaterialIcon>mail</MaterialIcon>{selected.email}</span>}{contact && <span><MaterialIcon>location_on</MaterialIcon>{contact}</span>}{selected.taxCode && <span><MaterialIcon>badge</MaterialIcon>{selected.taxCode}</span>}{!selected.phone && !selected.email && !contact && !selected.taxCode && <span className="muted">Nessun recapito registrato.</span>}</div><h3 className="customer-hist-title">Ultimi acquisti</h3>{!history ? <Empty>Caricamento…</Empty> : history.sales.length ? history.sales.map((sale) => <article className="history-sale" key={sale.id}><div><strong>{sale.receiptNo}</strong><small>{dateTime(sale.createdAt)} · {sale.store}</small></div><b className={sale.total < 0 ? "negative" : ""}>{money(sale.total)}</b><a className="secondary small" href={`/api/pdf?type=receipt&id=${sale.id}`}>Stampa scontrino interno</a><ul>{history.items.filter((item) => item.saleId === sale.id).map((item, index) => <li key={`${sale.id}-${index}`}>{item.quantity} × {item.description} <span>{money(item.lineTotal)}</span></li>)}</ul></article>) : <Empty>Nessuna vendita collegata a questo cliente.</Empty>}</div></Modal>}{newOpen && <Modal title="Nuovo cliente" onClose={() => setNewOpen(false)}><CustomerForm store={data.user.store ?? "Viterbo"} onSaved={reload} onClose={() => setNewOpen(false)} /></Modal>}{showImport && <Modal title="Importa clienti da CSV" wide onClose={() => setShowImport(false)}><ClientImport reload={reload} onClose={() => setShowImport(false)} /></Modal>}{editing && selected && <Modal title="Modifica cliente" onClose={() => setEditing(false)}><CustomerForm customer={selected} store={selected.createdStore} onSaved={async () => { await reload(); await openCustomer(selected); }} onClose={() => setEditing(false)} /></Modal>}</section>;
}
...
function ReorderList({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  ...
  const settings = data.reorderSettings ?? { globalMinEnabled: false, globalMin: 2, suggestTransfers: true, transferSafety: 0 };
  const isAdmin = data.user.role === "admin";
  ...
  const allRows = useMemo<ReorderRow[]>(() => data.products.flatMap((product) => {
    ...
    return stores.flatMap((currentStore) => {
      ...
      const minimum = settings.globalMinEnabled ? settings.globalMin : (currentStore === "Viterbo" ? product.viterboReorderLevel : product.granSassoReorderLevel);
      ...
      const otherMinimum = settings.globalMinEnabled ? settings.globalMin : (currentStore === "Viterbo" ? product.granSassoReorderLevel : product.viterboReorderLevel);
      const suggested = Math.max(1, minimum + 1 - available);
      const transferAmount = settings.suggestTransfers ? Math.min(suggested, Math.max(0, otherAvailable - otherMinimum - settings.transferSafety)) : 0;
  ...
  }).sort((left, right) => ({ urgent: 0, critical: 1, low: 2 }[left.priority] - { urgent: 0, critical: 1, low: 2 }[right.priority] || (left.available - left.minimum) - (right.available - right.minimum) || variantCollator.compare(`${left.product.brand} ${left.product.name} ${left.product.color} ${left.product.size}`, `${right.product.brand} ${right.product.name} ${right.product.color} ${right.product.size}`))), [data.products, store, settings.globalMinEnabled, settings.globalMin, settings.suggestTransfers, settings.transferSafety]);
  const rows = useMemo(() => {
    const normalized = query.trim().toLocaleLowerCase("it");
  ...
  return <section className="screen"><div className="screen-head"><div><p className="eyebrow">CONTROLLO SCORTE PER NEGOZIO</p><h1>Articoli da riordinare</h1><p className="muted inventory-intro">Ogni variante viene controllata separatamente a Viterbo e Gran Sasso usando la sua scorta minima.</p></div><div className="head-controls">{isAdmin && <button className="secondary small" onClick={() => setShowSettings(true)}><MaterialIcon>tune</MaterialIcon> Impostazioni</button>}<button className="primary" onClick={printList} disabled={!rows.length}><MaterialIcon>print</MaterialIcon>{visibleSelected.length ? `Stampa selezionati (${visibleSelected.length})` : `Stampa lista (${rows.length})`}</button></div></div><div className="metric-grid"><Metric label="Da riordinare" value={String(allRows.length)} note={store === "Tutti" ? "sotto la soglia · due negozi" : `sotto la soglia · ${store}`} /><Metric label="Trasferimenti suggeriti" value={String(allRows.filter((row) => row.transferAmount > 0).length)} note="disponibili dall'altro negozio" /></div><div className="panel reorder-panel"><div className="reorder-filters"><ClearableInput label="Cerca prodotto, variante o EAN" value={query} onChange={setQuery} placeholder="Marca, modello, colore, taglia…" /><label className="field"><span>Negozio</span><select value={store} onChange={(event) => { setStore(event.target.value as "Tutti" | Store); setSelected(new Set()); }}><option>Tutti</option><option>Viterbo</option><option>Gran Sasso</option></select></label><label className="field"><span>Priorità</span><select value={priority} onChange={(event) => setPriority(event.target.value as PriorityFilter)}><option>Tutte</option><option>Esauriti</option><option>Critici</option><option>Trasferibili</option></select></label><label className="field"><span>Marca</span><select value={brand} onChange={(event) => setBrand(event.target.value)}><option>Tutte</option>{brands.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field"><span>Categoria</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option>Tutte</option>{categories.map((item) => <option key={item}>{item}</option>)}</select></label></div><div className="reorder-toolbar"><button className="secondary small" onClick={toggleVisible} disabled={!rows.length}><MaterialIcon>{allVisibleSelected ? "deselect" : "select_all"}</MaterialIcon>{allVisibleSelected ? "Deseleziona tutto" : "Seleziona tutto"}</button><span>{rows.length} risultati · {visibleSelected.length} selezionati</span></div><div className="table-wrap"><table className="reorder-table"><thead><tr><th className="checkbox-column"><span className="sr-only">Selezione</span></th><th>Prodotto</th><th>Variante</th><th>Negozio</th><th>Disponibili</th><th>Minimo</th><th>Da ordinare</th><th>Azione consigliata</th><th>Priorità</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key} className={row.priority === "urgent" ? "reorder-urgent" : ""}><td><input className="row-checkbox" type="checkbox" checked={selected.has(row.key)} onChange={() => toggle(row.key)} aria-label={`Seleziona ${row.product.name} ${row.store}`} /></td><td><button type="button" className="text-button warehouse-group-name" onClick={() => setDetailGroup(row.product.variantGroup || `single-${row.product.id}`)} title="Apri scheda prodotto"><strong>{row.product.brand ? `${row.product.brand} · ` : ""}{row.product.name}</strong><small>{row.product.sku} · {row.product.eans.split(",").join(" · ")}</small></button></td><td><strong>{row.product.color}</strong><small>Taglia {row.product.size}</small></td><td><span className={`store-chip ${row.store === "Viterbo" ? "viterbo" : "gran-sasso"}`}>{row.store}</span></td><td><b className={row.available <= 0 ? "negative" : ""}>{row.available}</b></td><td>{row.minimum}</td><td><strong className="reorder-quantity">{row.suggested}</strong></td><td>{row.transferAmount > 0 ? <span className="transfer-tip"><MaterialIcon>swap_horiz</MaterialIcon>Trasferisci {row.transferAmount} da {row.otherStore}</span> : <span className="supplier-tip"><MaterialIcon>local_shipping</MaterialIcon>Ordine fornitore</span>}</td><td><span className={`status ${row.priority === "urgent" ? "reorder-now" : row.priority === "critical" ? "reorder-critical" : "reorder-soon"}`}>{row.priority === "urgent" ? "Esaurito" : row.priority === "critical" ? "Critico" : "Scorta bassa"}</span></td></tr>)}</tbody></table></div>{!rows.length && <Empty>Nessun articolo da riordinare con questi filtri.</Empty>}</div>{detailGroup && <Modal title="Scheda prodotto" wide guard={false} onClose={() => setDetailGroup(null)}><ProductDetail data={data} reload={reload} groupKey={detailGroup} onClose={() => setDetailGroup(null)} /></Modal>}{showSettings && <Modal title="Impostazioni riordino" onClose={() => setShowSettings(false)}><ReorderSettingsForm current={settings} reload={reload} onClose={() => setShowSettings(false)} /></Modal>}</section>;
}

function ReorderSettingsForm({ current, reload, onClose }: { current: { globalMinEnabled: boolean; globalMin: number; suggestTransfers: boolean; transferSafety: number }; reload: () => Promise<void>; onClose: () => void }) {
  const [globalMinEnabled, setGlobalMinEnabled] = useState(current.globalMinEnabled);
  const [globalMin, setGlobalMin] = useState(String(current.globalMin));
  const [suggestTransfers, setSuggestTransfers] = useState(current.suggestTransfers);
  const [transferSafety, setTransferSafety] = useState(String(current.transferSafety));
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  async function save() {
    setSaving(true); setError("");
    try {
      await post("saveReorderSettings", { globalMinEnabled, globalMin: Number(globalMin) || 0, suggestTransfers, transferSafety: Number(transferSafety) || 0 });
      await reload(); onClose();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
    finally { setSaving(false); }
  }
  return <div className="stack">
    <p className="muted">Regole valide solo per la pagina «Da riordinare». Restano attive anche nelle sessioni future, su tutti i dispositivi.</p>
    <label className="toggle-row"><input type="checkbox" checked={globalMinEnabled} onChange={(event) => setGlobalMinEnabled(event.target.checked)} /><span><strong>Soglia minima unica</strong><small>Usa un unico minimo per tutti i prodotti, ignorando le scorte minime impostate per singolo prodotto.</small></span></label>
    {globalMinEnabled && <ClearableInput label="Minimo per considerare un articolo «da riordinare»" type="number" min="0" step="1" value={globalMin} onChange={setGlobalMin} />}
    <label className="toggle-row"><input type="checkbox" checked={suggestTransfers} onChange={(event) => setSuggestTransfers(event.target.checked)} /><span><strong>Suggerisci trasferimenti</strong><small>Se l&apos;altro negozio ha esubero, proponi un trasferimento invece dell&apos;ordine al fornitore.</small></span></label>
    {suggestTransfers && <ClearableInput label="Pezzi da lasciare all&apos;altro negozio" type="number" min="0" step="1" value={transferSafety} onChange={setTransferSafety} />}
    {error && <div className="alert danger">{error}</div>}
    <div className="form-actions"><button type="button" className="secondary" onClick={onClose}>Annulla</button><button type="button" className="primary" disabled={saving} onClick={save}>{saving ? "Salvataggio…" : "Salva impostazioni"}</button></div>
  </div>;
}

function Transfers({ data, reload, newResponses = [] }: { data: Bootstrap; reload: () => Promise<void>; newResponses?: Transfer[] }) {
  const isAdmin = data.user.role === "admin";
  const [banner] = useState<Transfer[]>(newResponses);
  const otherStore = (s: Store): Store => s === "Viterbo" ? "Gran Sasso" : "Viterbo";
  const [fromStore, setFromStore] = useState<Store>(isAdmin ? "Viterbo" : otherStore(data.user.store ?? "Viterbo"));
  const [items, setItems] = useState<CartItem[]>([]);
  const [note, setNote] = useState("");
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");
  const [editing, setEditing] = useState<{ transfer: Transfer; items: TransferAdminItem[] } | null>(null);
  const toStore: Store = otherStore(fromStore);
  ...
  async function submit() {
    setError(""); setMessage("");
    try { const result = await post("createTransfer", { fromStore, items, note }); setItems([]); setNote(""); setMessage(`Richiesta ${result.code} inviata. In attesa di conferma dell'amministratore.`); await reload(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
  }
  ...
  const myTransfers = isAdmin ? history : data.transfers;
  return <section className="screen"><div className="screen-head"><div><h1>Trasferimenti</h1><p className="muted inventory-intro">Richiedi merce dall&apos;altro negozio; l&apos;amministratore conferma e la giacenza si aggiorna.</p></div></div>{message && <div className="alert success">{message}</div>}{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}
    {!isAdmin && banner.map((transfer) => transfer.status === "accepted"
      ? <div className="alert success visual-alert" key={transfer.id}><MaterialIcon>check_circle</MaterialIcon>Trasferimento {transfer.code} autorizzato: la merce è stata spostata verso il tuo negozio.</div>
      : <div className="alert danger visual-alert" key={transfer.id}><MaterialIcon>cancel</MaterialIcon>Trasferimento {transfer.code} rifiutato{transfer.note ? ` · ${transfer.note}` : ""}.</div>)}
    {isAdmin && <div className="panel"><div className="panel-title"><div><h2>Richieste in arrivo</h2></div>{pending.length > 0 && <span className="count-pill">{pending.length}</span>}</div>{pending.length ? <div className="table-wrap"><table><thead><tr><th>Codice</th><th>Data</th><th>Tratta</th><th>Articoli</th><th>Nota</th><th>Azioni</th></tr></thead><tbody>{pending.map((transfer) => <tr key={transfer.id}><td>{transfer.code}</td><td>{dateTime(transfer.createdAt)}</td><td>{transfer.fromStore} → {transfer.toStore}</td><td>{transfer.totalQuantity} pz · {transfer.lineCount} righe</td><td>{transfer.note || "—"}</td><td><div className="table-actions"><button className="primary small" onClick={() => void complete(transfer)}>Completa</button><button className="text-button negative" onClick={() => void reject(transfer)}>Rifiuta</button></div></td></tr>)}</tbody></table></div> : <Empty>Nessuna richiesta in attesa.</Empty>}</div>}
    <div className="transfer-grid"><div className="panel"><div className="panel-title"><div><h2>Nuova richiesta</h2></div></div><div className="transfer-route">{isAdmin ? <><label className="field"><span>Da</span><select value={fromStore} onChange={(event) => setFromStore(event.target.value as Store)}><option>Viterbo</option><option>Gran Sasso</option></select></label><span className="transfer-route-arrow"><MaterialIcon>arrow_forward</MaterialIcon></span><label className="field"><span>A</span><select value={toStore} onChange={(event) => setFromStore(otherStore(event.target.value as Store))}><option>Viterbo</option><option>Gran Sasso</option></select></label></> : <><label className="field"><span>Da</span><select value={fromStore} onChange={(event) => setFromStore(event.target.value as Store)}><option>{otherStore(data.user.store ?? "Viterbo")}</option></select></label><span className="transfer-route-arrow"><MaterialIcon>arrow_forward</MaterialIcon></span><div className="field"><span>A (il tuo negozio)</span><div className="transfer-fixed"><span className={`dot ${toStore === "Viterbo" ? "viterbo" : "gran-sasso"}`} />{toStore}</div></div></>}</div><p className="muted" style={{ margin: ".4rem 0 .8rem" }}>La merce verrà spostata da <strong>{fromStore}</strong> a <strong>{toStore}</strong>.</p><ProductSearch compact products={data.products} store={fromStore} onAdd={add} /><div className="cart-list transfer-lines">{items.map((item) => { const a = avail(item.productId); const danger = item.quantity > a; return <div className={`cart-row ${danger ? "insufficient-row" : ""}`} key={item.key}><div className="cart-desc"><strong>{item.description}</strong><small>Disponibili a {fromStore}: {a}</small></div><ClearableInput compact type="number" min="1" value={item.quantity} onChange={(value) => setItems((current) => current.map((row) => row.key === item.key ? { ...row, quantity: Number(value) || 0 } : row))} /><button className="icon-button" onClick={() => setItems((current) => current.filter((row) => row.key !== item.key))}><MaterialIcon>close</MaterialIcon></button></div>; })}{!items.length && <Empty>Aggiungi i prodotti da trasferire.</Empty>}</div></div><aside className="panel"><div className="panel-title"><div><h2>Riepilogo</h2></div></div><div className="stack"><p className="muted">Da <strong>{fromStore}</strong> → a <strong>{toStore}</strong></p><label className="field"><span>Nota (facoltativa)</span><textarea rows={3} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Es. Servono per il weekend" style={{ width: "100%", padding: ".8rem 1rem", borderRadius: 14, border: "1px solid #cfe0ea", font: "inherit" }} /></label><button className="primary" disabled={!items.length} onClick={submit}><MaterialIcon>send</MaterialIcon> Invia richiesta</button></div></aside></div>
    <div className="panel"><div className="panel-title"><div><h2>{isAdmin ? "Trasferimenti" : "Le mie richieste"}</h2></div></div>{myTransfers.length ? <div className="table-wrap"><table><thead><tr><th>Codice</th><th>Data</th><th>Tratta</th><th>Qtà</th><th>Stato</th><th>DDT</th>{isAdmin && <th>Azioni</th>}</tr></thead><tbody>{myTransfers.map((transfer) => <tr key={transfer.id} className={transfer.status === "accepted" ? "transfer-accepted" : transfer.status === "rejected" ? "transfer-rejected" : ""}><td>{transfer.code}</td><td>{dateTime(transfer.completedAt || transfer.createdAt)}</td><td>{transfer.fromStore} → {transfer.toStore}</td><td>{transfer.totalQuantity}</td><td>{statusBadge(transfer.status)}{transfer.status === "rejected" && transfer.note ? <small>{transfer.note}</small> : null}</td><td>{transfer.status === "accepted" ? <a href={`/api/pdf?type=ddt&id=${transfer.id}`}>PDF</a> : "—"}</td>{isAdmin && <td><div className="table-actions">{transfer.status === "accepted" && <button className="secondary small" onClick={() => void editTransfer(transfer)}>Modifica</button>}<button className="text-button negative" onClick={() => void removeTransfer(transfer)}>Rimuovi</button></div></td>}</tr>)}</tbody></table></div> : <Empty>Nessun trasferimento.</Empty>}</div>
...
function NewProductForm({ data, reload, onCreated }: { data: Bootstrap; reload: () => Promise<void>; onCreated?: () => void }) {
  type VariantDraft = { key: string; sku: string; skuTouched: boolean; color: string; size: string; eans: string; viterboQty: string; granSassoQty: string; photo: File | null; photoPreview: string };
  const emptyVariant = (): VariantDraft => ({ key: keyId(), sku: "", skuTouched: false, color: "", size: "", eans: "", viterboQty: "0", granSassoQty: "0", photo: null, photoPreview: "" });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ name: "", brand: "", category: "", price: "" });
  const [variants, setVariants] = useState<VariantDraft[]>([emptyVariant()]);
  const showVt = data.user.role === "admin" || data.user.store === "Viterbo";
  const showGs = data.user.role === "admin" || data.user.store === "Gran Sasso";
  const slug = (value: string) => value.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 10);
  const makeSku = (color: string, size: string) => [form.name, color, size].map(slug).filter(Boolean).join("-").toUpperCase();
  const change = (name: keyof typeof form) => (value: string) => setForm((current) => ({ ...current, [name]: value }));
  const setV = (key: string, patch: Partial<VariantDraft>) => setVariants((current) => current.map((v) => v.key === key ? { ...v, ...patch } : v));
  function setColor(key: string, value: string) { setVariants((current) => current.map((v) => { if (v.key !== key) return v; return { ...v, color: value, sku: v.skuTouched ? v.sku : [form.name, value, v.size].map(slug).filter(Boolean).join("-").toUpperCase() }; })); }
  function setSize(key: string, value: string) { setVariants((current) => current.map((v) => { if (v.key !== key) return v; return { ...v, size: value, sku: v.skuTouched ? v.sku : [form.name, v.color, value].map(slug).filter(Boolean).join("-").toUpperCase() }; })); }
  function choosePhoto(key: string, file: File | null) {
    if (!file) { setV(key, { photo: null, photoPreview: "" }); return; }
  ...
  async function create(event: React.FormEvent) {
    ...
    try {
      ...
      await readJson(await fetch("/api/products", { method: "POST", body }));
      setForm({ name: "", brand: "", category: "", price: "" }); setVariants([emptyVariant()]);
      await reload(); if (onCreated) onCreated(); else setError(`"${createdName}" creato.`);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
    finally { setSaving(false); }
  }
  return <div className="np-form"><div className="panel-title"><div><p className="eyebrow">NUOVO PRODOTTO</p><h2>{form.name || "Nuovo prodotto"}</h2></div><span className="count-pill">{variants.length} {variants.length === 1 ? "variante" : "varianti"}</span></div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<form onSubmit={create}><div className="form-grid product-base-fields"><ClearableInput label="Nome prodotto" value={form.name} onChange={change("name")} required autoFocus /><ClearableInput label="Marca" value={form.brand} onChange={change("brand")} required /><ClearableInput label="Categoria" value={form.category} onChange={change("category")} required /><ClearableInput label="Prezzo di vendita" type="number" min="0" step="0.01" value={form.price} onChange={change("price")} required /></div><div className="variant-builder-head"><p className="eyebrow">VARIANTI (colore/taglia)</p><button type="button" className="secondary small" onClick={() => setVariants((current) => [...current, emptyVariant()])}><MaterialIcon>add</MaterialIcon> Aggiungi variante</button></div><div className="nv-head"><span>Colore</span><span>Taglia</span><span>SKU</span><span>EAN</span>{showVt && <span>Giac. VT</span>}{showGs && <span>Giac. GS</span>}<span>Foto</span><span /></div><div className="nv-list">{variants.map((v) => <div className="nv-row" key={v.key}><input className="nv-in" placeholder="Colore" value={v.color} onChange={(e) => setColor(v.key, e.target.value)} required /><input className="nv-in nv-sm" placeholder="Taglia" value={v.size} onChange={(e) => setSize(v.key, e.target.value)} required /><span className="nv-with-btn"><input className="nv-in" placeholder="SKU" value={v.sku} onChange={(e) => setV(v.key, { sku: e.target.value, skuTouched: true })} required /><button type="button" className="nv-btn" title="Genera SKU dal nome" onClick={() => setV(v.key, { sku: makeSku(v.color, v.size), skuTouched: false })}><MaterialIcon>autorenew</MaterialIcon></button></span><span className="nv-with-btn"><input className="nv-in" placeholder="EAN" value={v.eans} onChange={(e) => setV(v.key, { eans: e.target.value })} required /><button type="button" className="nv-btn" title="Genera un EAN" onClick={() => setV(v.key, { eans: v.eans ? `${v.eans}, ${makeEan13()}` : makeEan13() })}><MaterialIcon>add</MaterialIcon></button></span>{showVt && <input className="nv-in nv-sm" type="number" min="0" placeholder="0" value={v.viterboQty} onChange={(e) => setV(v.key, { viterboQty: e.target.value })} />}{showGs && <input className="nv-in nv-sm" type="number" min="0" placeholder="0" value={v.granSassoQty} onChange={(e) => setV(v.key, { granSassoQty: e.target.value })} />}<label className="nv-photo" title="Foto colore (opzionale)">{v.photoPreview ? <img src={v.photoPreview} alt="" /> : <MaterialIcon>photo_camera</MaterialIcon>}<input type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={(e) => choosePhoto(v.key, e.target.files?.[0] ?? null)} /></label>{variants.length > 1 ? <button type="button" className="icon-button" aria-label="Rimuovi variante" onClick={() => setVariants((current) => current.filter((x) => x.key !== v.key))}><MaterialIcon>close</MaterialIcon></button> : <span />}</div>)}</div><div className="form-actions"><button className="primary" disabled={saving}>{saving ? "Salvataggio…" : `Salva prodotto (${variants.length} ${variants.length === 1 ? "variante" : "varianti"})`}</button></div></form></div>;
}
...
const CLIENTS_TEMPLATE = "cognome,nome,telefono,email,indirizzo,cap,comune,provincia,codice_fiscale\nRossi,Marco,3331234567,marco.rossi@email.it,Via Roma 1,01100,Viterbo,VT,\n";
function ClientImport({ reload, onClose }: { reload: () => Promise<void>; onClose: () => void }) {
  type Row = { lastName: string; firstName: string; phone: string; email: string; address: string; postalCode: string; city: string; province: string; taxCode: string; _valid: boolean };
  const [rows, setRows] = useState<Row[]>([]);
  const [error, setError] = useState("");
  const [result, setResult] = useState<{ created: number; skipped: { name: string; reason: string }[] } | null>(null);
  const [saving, setSaving] = useState(false);
  const map: Record<string, keyof Row> = { cognome: "lastName", lastname: "lastName", nome: "firstName", firstname: "firstName", name: "firstName", telefono: "phone", phone: "phone", cellulare: "phone", email: "email", mail: "email", indirizzo: "address", address: "address", cap: "postalCode", comune: "city", citta: "city", city: "city", provincia: "province", codice_fiscale: "taxCode", cf: "taxCode", codicefiscale: "taxCode" };
  function onFile(file: File | null) {
    ...
    reader.onload = () => {
      try {
        ...
        setRows(grid.slice(1).map((cols) => {
          const r = { lastName: "", firstName: "", phone: "", email: "", address: "", postalCode: "", city: "", province: "", taxCode: "", _valid: false } as Row;
          header.forEach((h, idx) => { const key = map[h]; if (key) (r[key] as string) = (cols[idx] ?? "").trim(); });
          r._valid = !!(r.lastName && r.firstName);
          return r;
        }));
  ...
  async function confirm() {
    setSaving(true); setError("");
    try { const res = await post("importCustomers", { rows: valid.map(({ _valid, ...rest }) => { void _valid; return rest; }) }); setResult(res as { created: number; skipped: { name: string; reason: string }[] }); await reload(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
    finally { setSaving(false); }
  }
  if (result) return <div className="stack"><div className="alert success"><MaterialIcon>check_circle</MaterialIcon> Import completato: <strong>{result.created}</strong> clienti{result.skipped.length ? `, ${result.skipped.length} saltati` : ""}.</div>{result.skipped.length > 0 && <div className="table-wrap import-preview"><table><thead><tr><th>Cliente</th><th>Motivo</th></tr></thead><tbody>{result.skipped.map((sk, i) => <tr key={i}><td>{sk.name}</td><td>{sk.reason}</td></tr>)}</tbody></table></div>}<div className="form-actions"><button className="primary" onClick={onClose}>Chiudi</button></div></div>;
  return <div className="stack"><div className="import-help">Carica un file <strong>CSV</strong> con colonne: <code>cognome, nome, telefono, email, indirizzo, cap, comune, provincia, codice_fiscale</code>. Obbligatori solo cognome e nome. <a href={`data:text/csv;charset=utf-8,${encodeURIComponent(CLIENTS_TEMPLATE)}`} download="modello-clienti.csv">Scarica il modello</a>.</div><label className="field"><span>File CSV</span><input type="file" accept=".csv,text/csv" onChange={(e) => onFile(e.target.files?.[0] ?? null)} /></label>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}{rows.length > 0 && <><div className="import-summary"><span>Righe: {rows.length}</span><span className="stock-ok">Validi: {valid.length}</span>{rows.length - valid.length > 0 && <span className="negative">Da correggere: {rows.length - valid.length}</span>}</div><div className="table-wrap import-preview"><table><thead><tr><th></th><th>Cognome</th><th>Nome</th><th>Telefono</th><th>Email</th><th>Comune</th></tr></thead><tbody>{rows.slice(0, 100).map((r, i) => <tr key={i} className={r._valid ? "" : "import-row-bad"}><td>{r._valid ? "OK" : "!"}</td><td>{r.lastName}</td><td>{r.firstName}</td><td>{r.phone}</td><td>{r.email}</td><td>{r.city}</td></tr>)}</tbody></table></div></>}<div className="form-actions"><button type="button" className="secondary" onClick={onClose}>Annulla</button><button type="button" className="primary" disabled={!valid.length || saving} onClick={() => void confirm()}>{saving ? "Importazione…" : `Importa ${valid.length} clienti`}</button></div></div>;
}
...
function Warehouse({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  ...
  const csvEscape = (value: unknown) => { const s = String(value ?? ""); return /[",;\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s; };
  const htmlEscape = (value: unknown) => String(value ?? "").replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c] as string));
  function exportCsv() {
    const header = ["nome", "marca", "categoria", "prezzo", "colore", "taglia", "sku", "ean", "viterbo", "gran_sasso"];
    const lines = [header.join(",")];
    for (const g of groupList) for (const it of g.items) lines.push([it.name, it.brand, it.category, it.price, it.color, it.size, it.sku, it.eans.split(",").map((e) => e.trim()).filter(Boolean).join(" "), it.viterboQty, it.granSassoQty].map(csvEscape).join(","));
    const a = document.createElement("a");
    a.href = "data:text/csv;charset=utf-8," + encodeURIComponent("﻿" + lines.join("\n"));
    a.download = `magazzino-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  }
  function printList() {
    ...
    const rows = groupList.flatMap((g) => g.items.map((it) => `<tr><td>${htmlEscape(it.name)}</td><td>${htmlEscape(it.brand)}</td><td>${htmlEscape(it.category)}</td><td>${htmlEscape(it.color)} · ${htmlEscape(it.size)}</td><td>${htmlEscape(it.sku)}</td><td class="r">${money(it.price)}</td><td class="r">${it.viterboQty}</td><td class="r">${it.granSassoQty}</td></tr>`)).join("");
    const count = groupList.reduce((sum, g) => sum + g.items.length, 0);
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Magazzino</title><style>body{font-family:Arial,Helvetica,sans-serif;padding:24px;color:#111}header{display:flex;align-items:center;gap:14px;border-bottom:2px solid #111;padding-bottom:12px;margin-bottom:16px}header img{height:52px}h1{font-size:18px;margin:0}small{color:#555}table{width:100%;border-collapse:collapse;font-size:12px}th,td{text-align:left;padding:6px 8px;border-bottom:1px solid #ddd}th{background:#f3f3f3}td.r,th.r{text-align:right}@media print{body{padding:0}}</style></head><body><header><img src="${window.location.origin}/ms-logo.png" alt=""><div><h1>Magazzino${normalized ? ` — filtro: "${htmlEscape(query)}"` : ""}</h1><small>${today} · ${count} articoli</small></div></header><table><thead><tr><th>Prodotto</th><th>Marca</th><th>Categoria</th><th>Variante</th><th>SKU</th><th class="r">Prezzo</th><th class="r">VT</th><th class="r">GS</th></tr></thead><tbody>${rows}</tbody></table></body></html>`);
    win.document.close(); win.focus(); win.print();
  }
  const emptyBlock = noProducts ? <EmptyGuide icon="inventory_2" title="Magazzino vuoto" text="Aggiungi il primo prodotto oppure importa il catalogo da un file CSV."><button className="primary" onClick={() => setShowNew(true)}><MaterialIcon>add_box</MaterialIcon> Nuovo prodotto</button><button className="secondary" onClick={() => setShowImport(true)}><MaterialIcon>upload_file</MaterialIcon> Importa CSV</button></EmptyGuide> : <Empty>Nessun prodotto trovato.</Empty>;
  return <section className="screen"><div className="screen-head"><div><p className="eyebrow">GESTIONE MAGAZZINO</p><h1>Magazzino</h1><p className="muted inventory-intro">Sfoglia prodotti e giacenze. Per modificare apri la scheda.</p></div><div className="head-controls"><div className="excel-menu">{excelOpen && <button className="excel-backdrop" aria-label="Chiudi" onClick={() => setExcelOpen(false)} />}<button className="secondary" onClick={() => setExcelOpen((value) => !value)}><MaterialIcon>table_view</MaterialIcon> Excel <MaterialIcon>{excelOpen ? "expand_less" : "expand_more"}</MaterialIcon></button>{excelOpen && <div className="excel-dropdown"><button type="button" onClick={() => { setShowImport(true); setExcelOpen(false); }}><MaterialIcon>upload_file</MaterialIcon> Importa CSV</button><button type="button" onClick={() => { exportCsv(); setExcelOpen(false); }}><MaterialIcon>download</MaterialIcon> Esporta CSV</button></div>}</div><button className="secondary" onClick={printList}><MaterialIcon>print</MaterialIcon> Stampa</button><button className="primary" onClick={() => setShowQuickLoad(true)}><MaterialIcon>barcode_scanner</MaterialIcon> Carico rapido</button><button className="primary" onClick={() => setShowNew(true)}><MaterialIcon>add_box</MaterialIcon> Nuovo prodotto</button></div></div><div className="panel"><div className="wh-toolbar"><div className="wh-toolbar-left"><div className="store-selector">{(["Tutti", "Viterbo", "Gran Sasso"] as const).map((option) => <button key={option} className={store === option ? "active" : ""} onClick={() => setStore(option)}>{option !== "Tutti" && <span className={`dot ${option === "Viterbo" ? "viterbo" : "gran-sasso"}`} />}{option}</button>)}</div></div><div className="view-toggle"><button className={view === "list" ? "active" : ""} onClick={() => setView("list")} title="Elenco"><MaterialIcon>view_list</MaterialIcon></button><button className={view === "grid" ? "active" : ""} onClick={() => setView("grid")} title="Griglia"><MaterialIcon>grid_view</MaterialIcon></button></div></div><div className="search-hero"><MaterialIcon>search</MaterialIcon><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cerca per nome, marca, colore, taglia, SKU o EAN…" /></div></div>{view === "grid" ? <div className="product-grid">{groupList.map((group) => <button key={group.key} type="button" className={`product-card ${groupLow(group) ? "low" : ""}`} onClick={() => setDetailGroup(group.key)}><ProductThumb photoKey={group.photoKey} name={group.name} className="thumb" /><div className="body"><span className="p-name">{group.name}</span><span className="p-meta">{group.brand} · {group.category} · {group.items.length} {group.items.length === 1 ? "variante" : "varianti"}</span><div className="p-foot"><span className="p-price">{money(group.price)}</span><span className="p-stock">{stockText(group)}{groupLow(group) && " ⚠"}</span></div></div></button>)}{!groupList.length && emptyBlock}</div> : <div className="panel"><div className="table-wrap"><table><thead><tr><th>Prodotto / Variante</th><th>SKU / EAN</th><th>Prezzo</th>{showVt && <th>Viterbo</th>}{showGs && <th>Gran Sasso</th>}</tr></thead><tbody>{groupList.map((group) => <Fragment key={group.key}><tr className="warehouse-group-row"><td colSpan={colCount}><button type="button" className="text-button" title="Espandi/comprimi" onClick={() => setExpanded((current) => ({ ...current, [group.key]: !current[group.key] }))}><MaterialIcon>{expanded[group.key] ? "expand_more" : "chevron_right"}</MaterialIcon></button><button type="button" className="text-button warehouse-group-name" onClick={() => setDetailGroup(group.key)} title="Apri scheda prodotto"><strong>{group.name}</strong> <small>{group.brand} · {group.category} · {money(group.price)} · {group.items.length} {group.items.length === 1 ? "variante" : "varianti"}</small></button><button className="secondary small" onClick={() => setDetailGroup(group.key)}>Apri scheda</button></td></tr>{expanded[group.key] && group.items.map((item) => <tr key={item.id} className={needsReorder(item) ? "warehouse-low" : ""}><td><strong>{item.color} · {item.size}</strong></td><td>{item.sku}<small>{item.eans.split(",").join(" · ")}</small></td><td>{money(item.price)}</td>{showVt && <td>{item.viterboQty}{item.viterboReserved > 0 && <small>{item.viterboReserved} pren.</small>}</td>}{showGs && <td>{item.granSassoQty}{item.granSassoReserved > 0 && <small>{item.granSassoReserved} pren.</small>}</td>}</tr>)}</Fragment>)}</tbody></table></div>{!groupList.length && emptyBlock}</div>}{showNew && <Modal title="Nuovo prodotto" onClose={() => setShowNew(false)}><NewProductForm data={data} reload={reload} onCreated={() => setShowNew(false)} /></Modal>}{showImport && <Modal title="Importa prodotti da CSV" wide onClose={() => setShowImport(false)}><ProductImport reload={reload} onClose={() => setShowImport(false)} /></Modal>}{showQuickLoad && <Modal title="Carico rapido (scansione EAN)" onClose={() => setShowQuickLoad(false)}><p className="muted" style={{ marginTop: 0 }}>Scansiona o digita l&apos;EAN di un prodotto <strong>già presente</strong> per aggiungere pezzi alla giacenza. Non crea nuovi prodotti.</p><EanLoadBar data={data} reload={reload} /></Modal>}{detailGroup && <Modal title="Scheda prodotto" wide guard={false} onClose={() => setDetailGroup(null)}><ProductDetail data={data} reload={reload} groupKey={detailGroup} onClose={() => setDetailGroup(null)} /></Modal>}</section>;
}
...
function GiftSummary({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  ...
  const isAdmin = data.user.role === "admin";
  const [firstName, setFirstName] = useState(""); const [lastName, setLastName] = useState(""); const [amount, setAmount] = useState(""); const [expiresAt, setExpiresAt] = useState(""); const [store, setStore] = useState<Store>(data.user.store ?? "Viterbo"); const [saving, setSaving] = useState(false); const [issued, setIssued] = useState<{ id: number; code: string } | null>(null); const [showForm, setShowForm] = useState(false);
  ...
  async function issue(event: React.FormEvent) {
    ...
    if (isClientTestMode()) { setError("Modalità TEST attiva: buono non creato."); return; }
    setSaving(true);
    try { const beneficiary = `${firstName} ${lastName}`.trim(); const result = await post("createGift", { beneficiary, value: Number(amount) || 0, expiresAt, store }); setIssued({ id: result.id, code: result.code }); setFirstName(""); setLastName(""); setAmount(""); setExpiresAt(""); setShowForm(false); await reload(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
    finally { setSaving(false); }
  }
  ...
  return <section className="screen"><div className="screen-head"><div><h1>Buoni regalo</h1><p className="muted inventory-intro">Genera, stampa e cerca i buoni regalo. Sono spendibili in cassa scansionando il codice.</p></div><div className="head-controls"><button className="primary" onClick={() => { setIssued(null); setShowForm(true); }}><MaterialIcon>add_card</MaterialIcon> Genera buono</button></div></div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}{issued && <div className="alert success visual-alert"><MaterialIcon>check_circle</MaterialIcon>Buono <strong>{issued.code}</strong> creato. <a href={`/api/pdf?type=gift&id=${issued.id}`} target="_blank" rel="noreferrer">Stampa PDF</a></div>}
    ...
    {showForm && <Modal title="Genera nuovo buono" onClose={() => setShowForm(false)}><form className="form-grid" onSubmit={issue}><ClearableInput label="Nome" value={firstName} onChange={setFirstName} required autoFocus /><ClearableInput label="Cognome" value={lastName} onChange={setLastName} required /><ClearableInput label="Importo" type="number" min="0" step="0.01" value={amount} onChange={setAmount} required /><ClearableInput label="Scadenza (facoltativa)" type="date" value={expiresAt} onChange={setExpiresAt} /><label className="field full"><span>Negozio emittente</span><select value={store} onChange={(event) => setStore(event.target.value as Store)}><option>Viterbo</option><option>Gran Sasso</option></select></label><div className="form-actions full"><button type="button" className="secondary" onClick={() => setShowForm(false)}>Annulla</button><button className="primary" disabled={saving}>{saving ? "Creazione…" : "Genera buono"}</button></div></form></Modal>}
    <div className="panel"><div className="search-hero"><MaterialIcon>search</MaterialIcon><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cerca per codice o intestatario…" /></div><div className="table-wrap"><table><thead><tr><th>Codice</th><th>Intestatario</th><th>Sede</th><th>Emesso</th><th>Residuo</th><th>Scadenza</th><th>Stato</th><th>Stampa</th>{isAdmin && <th>Azioni</th>}</tr></thead><tbody>{visible.length ? visible.map((gift) => <tr key={gift.id}><td>{gift.code}</td><td>{gift.beneficiary}</td><td>{gift.store}</td><td>{money(gift.initialValue)}</td><td><b>{money(gift.balance)}</b></td><td>{gift.expiresAt ? new Date(gift.expiresAt).toLocaleDateString("it-IT") : "—"}</td><td><span className={`status ${gift.status}`}>{gift.status === "reversed" ? "stornato" : gift.status}</span></td><td><a href={`/api/pdf?type=gift&id=${gift.id}`} target="_blank" rel="noreferrer">Stampa</a></td>{isAdmin && <td><div className="table-actions"><button className="secondary small" onClick={() => setEditing(gift)}>Modifica</button><button className="text-button negative" onClick={() => void remove(gift)}>Rimuovi</button></div></td>}</tr>) : <tr><td colSpan={isAdmin ? 9 : 8}><Empty>Nessun buono trovato.</Empty></td></tr>}</tbody></table></div></div>{editing && <Modal title="Modifica buono" onClose={() => setEditing(null)}><GiftAdminForm gift={editing} reload={reload} close={() => setEditing(null)} /></Modal>}</section>;
}

function ReservationSummary({ data, reload, repair = false, goToCash }: { data: Bootstrap; reload: () => Promise<void>; repair?: boolean; goToCash?: (action: PendingCashAction) => void }) {
  const [query, setQuery] = useState(""); const [editing, setEditing] = useState<Reservation | null>(null); const [error, setError] = useState("");
  const isAdmin = data.user.role === "admin";
  const visible = data.reservations.filter((reservation) => (repair ? reservation.kind === "repair" : reservation.kind !== "repair") && (data.user.role === "admin" || reservation.store === data.user.store) && `${reservation.code} ${reservation.description} ${reservation.itemDescriptions} ${reservation.customerName}`.toLowerCase().includes(query.toLowerCase()));
  const [showNew, setShowNew] = useState(false);
  const keyRes = () => `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  // Acconto già incassato: sia dal flag (creata da pagina + incassato) sia se
  // creata in cassa (issued_sale_id valorizzato = acconto preso alla creazione).
  const depositCollected = (r: Reservation) => r.depositPaid > 0 || !!r.issuedSaleId;
  function incassaAcconto(reservation: Reservation) {
    if (!goToCash) return;
    const noun = reservation.kind === "repair" ? "risuolatura" : "prenotazione";
    goToCash({ kind: "reservationDeposit", reservationId: reservation.id, label: `Incasso acconto ${noun} ${reservation.code}`, lines: [{ key: keyRes(), productId: null, description: `Acconto ${noun} ${reservation.code}`, quantity: 1, unitPrice: reservation.depositAmount, discountPercent: 0, itemType: "reservation_deposit", locked: true, metadata: { reservationId: reservation.id } }] });
  }
  async function consegna(reservation: Reservation) {
    ...
    const noun = reservation.kind === "repair" ? "risuolatura" : "prenotazione";
    try {
      const detail = await readJson(await fetch(`/api/data?view=code&q=${encodeURIComponent(reservation.code)}`, { cache: "no-store" }));
      ...
      const lines: ShellCartItem[] = items.map((row, index) => ({ key: `${keyRes()}-${index}`, productId: row.productId, description: row.description, quantity: row.quantity, unitPrice: row.unitPrice, discountPercent: row.discountPercent ?? 0, itemType: row.productId ? "product" : "service", metadata: index === 0 ? { deliverReservationId: reservation.id } : {} }));
      if (!lines.length) lines.push({ key: keyRes(), productId: null, description: reservation.description, quantity: 1, unitPrice: reservation.totalPrice, discountPercent: 0, itemType: "service", metadata: { deliverReservationId: reservation.id } });
      if (depositCollected(reservation) && reservation.depositAmount > 0) lines.push({ key: keyRes(), productId: null, description: `Acconto già versato ${reservation.code}`, quantity: 1, unitPrice: -reservation.depositAmount, discountPercent: 0, itemType: "reservation_credit", locked: true, metadata: { reservationId: reservation.id } });
      goToCash({ kind: "reservationDelivery", reservationId: reservation.id, label: `Consegna ${noun} ${reservation.code}`, lines });
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
  }
  async function cancel(reservation: Reservation) { if (!confirm(`Annullare ${reservation.kind === "repair" ? "la risuolatura" : "la prenotazione"} ${reservation.code}? La merce riservata verrà liberata.`)) return; setError(""); try { await post("cancelReservation", { id: reservation.id }); await reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
  async function remove(reservation: Reservation) { if (!confirm(`Cancellare ${repair ? "la risuolatura" : "la prenotazione"} ${reservation.code}?`)) return; setError(""); try { await post("deleteReservation", { id: reservation.id }); await reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
  return <section className="screen"><div className="screen-head"><div><h1>{repair ? "Risuolature" : "Prenotazioni"}</h1><p className="muted inventory-intro">{repair ? "Riparazioni affidate dai clienti, con acconto e saldo al ritiro." : "Articoli o servizi prenotati dai clienti, con acconto e saldo al ritiro."}</p></div><div className="head-controls"><button className="primary" onClick={() => setShowNew(true)}><MaterialIcon>add</MaterialIcon> {repair ? "Nuova risuolatura" : "Nuova prenotazione"}</button></div></div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<div className="panel"><ClearableInput label="Ricerca codice, prodotto o cliente" value={query} onChange={setQuery} /><div className="table-wrap"><table><thead><tr><th>Codice</th><th>Data</th><th>Sede</th><th>Descrizione</th><th>Articoli</th><th>Cliente</th><th>Acconto</th><th>Saldo</th><th>Stato</th><th>{repair ? "Ritiro" : "Consegna"}</th><th>PDF EAN</th><th>Azioni</th></tr></thead><tbody>{visible.map((reservation) => <tr key={reservation.id}><td>{reservation.code}</td><td>{dateTime(reservation.createdAt)}</td><td>{reservation.store}</td><td>{reservation.kind === "repair" ? <><strong>Risuolatura</strong><small>{reservation.itemDescriptions || reservation.description}</small></> : reservation.description}</td><td>{reservation.itemCount || 1}</td><td>{reservation.customerName || "Non associato"}</td><td>{money(reservation.depositAmount)}</td><td><b>{money(reservation.balanceDue)}</b></td><td><span className={`status ${reservation.status === "open" ? "active" : "used"}`}>{reservation.status}</span>{depositCollected(reservation) ? <span className="status active">acconto incassato</span> : reservation.depositAmount > 0 ? <span className="status warning">acconto da incassare</span> : null}</td><td>{reservation.expectedDelivery ? new Date(reservation.expectedDelivery).toLocaleDateString("it-IT") : "—"}</td><td><a href={`/api/pdf?type=reservation&id=${reservation.id}`}>Scarica</a></td><td><div className="table-actions">{reservation.status === "open" && reservation.depositAmount > 0 && !depositCollected(reservation) && <button className="secondary small" onClick={() => incassaAcconto(reservation)}>Incassa acconto</button>}{reservation.status === "open" && <button className="primary small" onClick={() => void consegna(reservation)}>Consegna</button>}{reservation.status === "open" && isAdmin && <button className="text-button negative" onClick={() => void cancel(reservation)}>Annulla</button>}{isAdmin && <><button className="secondary small" onClick={() => setEditing(reservation)}>Modifica</button><button className="text-button negative" onClick={() => void remove(reservation)}>Rimuovi</button></>}</div></td></tr>)}</tbody></table></div></div>{editing && <Modal title="Modifica prenotazione" onClose={() => setEditing(null)}><ReservationAdminForm reservation={editing} reload={reload} close={() => setEditing(null)} /></Modal>}{showNew && <Modal title={repair ? "Nuova risuolatura" : "Nuova prenotazione"} wide onClose={() => setShowNew(false)}><ReservationNewForm data={data} reload={reload} repair={repair} onClose={() => setShowNew(false)} /></Modal>}</section>;
}

function ReservationNewForm({ data, reload, onClose, repair = false }: { data: Bootstrap; reload: () => Promise<void>; onClose: () => void; repair?: boolean }) {
  const [type, setType] = useState<"product" | "service">("product");
  const effectiveType = repair ? "repair" : type;
  const [firstName, setFirstName] = useState(""); const [lastName, setLastName] = useState("");
  const [items, setItems] = useState<{ key: string; productId: number | null; description: string; quantity: number; unitPrice: number }[]>([]);
  const [serviceDescription, setServiceDescription] = useState(""); const [serviceAmount, setServiceAmount] = useState("");
  const [repairDesc, setRepairDesc] = useState(""); const [repairQty, setRepairQty] = useState("1"); const [repairCost, setRepairCost] = useState("");
  ...
  function addRepairLine() { const description = repairDesc.trim(); if (!description) return; setItems((c) => [...c, { key: `${Date.now()}-${c.length}`, productId: null, description, quantity: Math.max(1, Number(repairQty) || 1), unitPrice: Math.max(0, Number(repairCost) || 0) }]); setRepairDesc(""); setRepairQty("1"); setRepairCost(""); }
  const total = effectiveType === "service" ? Number(serviceAmount) || 0 : items.reduce((s, i) => s + i.quantity * i.unitPrice, 0);
  const noun = repair ? "risuolatura" : "prenotazione";
  async function save(event: React.FormEvent) {
    event.preventDefault(); setError("");
    if (isClientTestMode()) { setError(`Modalità TEST attiva: ${noun} non salvata.`); return; }
    setSaving(true);
    try {
      await post("createReservation", { type: effectiveType, firstName, lastName, items: items.map(({ productId, description, quantity, unitPrice }) => ({ productId, description, quantity, unitPrice })), serviceDescription, serviceAmount: Number(serviceAmount) || 0, depositAmount: Number(deposit) || 0, expectedDelivery, note, store });
      await reload(); onClose();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
    finally { setSaving(false); }
  }
  return <form className="stack" onSubmit={save}>
    ...
    {repair ? <><div className="repair-entry-grid"><ClearableInput label="Articolo da risuolare" value={repairDesc} onChange={setRepairDesc} placeholder="Es. Scarponcino marrone uomo" /><ClearableInput label="Quantità" type="number" min="1" value={repairQty} onChange={setRepairQty} /><ClearableInput label="Costo" type="number" min="0" step="0.01" value={repairCost} onChange={setRepairCost} /><button type="button" className="secondary align-end" onClick={addRepairLine}><MaterialIcon>add</MaterialIcon> Aggiungi</button></div><div className="cart-list">{items.map((item) => <div className="cart-row" key={item.key}><div className="cart-desc"><strong>{item.description}</strong><small>{item.quantity} × {money(item.unitPrice)}</small></div><b>{money(item.quantity * item.unitPrice)}</b><button type="button" className="icon-button" onClick={() => setItems((c) => c.filter((r) => r.key !== item.key))}><MaterialIcon>close</MaterialIcon></button></div>)}{!items.length && <Empty>Aggiungi gli articoli da risuolare.</Empty>}</div></>
    : <><ProductSearch compact products={data.products} store={store} onAdd={addProduct} /><div className="cart-list">{items.map((item) => <div className="cart-row" key={item.key}><div className="cart-desc"><strong>{item.description}</strong></div><ClearableInput compact type="number" min="1" value={item.quantity} onChange={(v) => setItems((c) => c.map((r) => r.key === item.key ? { ...r, quantity: Number(v) || 1 } : r))} /><ClearableInput compact type="number" min="0" step="0.01" value={item.unitPrice} onChange={(v) => setItems((c) => c.map((r) => r.key === item.key ? { ...r, unitPrice: Number(v) || 0 } : r))} /><button type="button" className="icon-button" onClick={() => setItems((c) => c.filter((r) => r.key !== item.key))}><MaterialIcon>close</MaterialIcon></button></div>)}{!items.length && <Empty>Aggiungi i prodotti da prenotare.</Empty>}</div></>}
    <div className="form-grid"><ClearableInput label="Acconto (facoltativo)" type="number" min="0" step="0.01" value={deposit} onChange={setDeposit} /><ClearableInput label={repair ? "Ritiro previsto (facoltativo)" : "Consegna prevista (facoltativa)"} type="date" value={expectedDelivery} onChange={setExpectedDelivery} /><label className="field"><span>Negozio</span><select value={store} onChange={(e) => setStore(e.target.value as Store)}><option>Viterbo</option><option>Gran Sasso</option></select></label></div>
    <ClearableInput className="full" label="Note (facoltative)" value={note} onChange={setNote} />
    <div className="total-line"><span>Totale {noun}</span><b>{money(total)}</b></div>
    {error && <div className="alert danger">{error}</div>}
    <div className="form-actions"><button type="button" className="secondary" onClick={onClose}>Annulla</button><button className="primary" disabled={saving || total <= 0}>{saving ? "Salvataggio…" : `Crea ${noun}`}</button></div>
  </form>;
}
...
function Register({ data }: { data: Bootstrap }) {
  type SortKey = "createdAt" | "store" | "receiptNo" | "customerName" | "total";
  const [query, setQuery] = useState(""); const [sortKey, setSortKey] = useState<SortKey>("createdAt"); const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const ownStore = data.user.store;
  const filtered = data.sales.filter((sale) => `${sale.receiptNo} ${sale.customerName} ${sale.store}`.toLowerCase().includes(query.toLowerCase()));
  const visible = useMemo(() => [...filtered].sort((a, b) => {
    const r = sortKey === "total" ? a.total - b.total
      : sortKey === "createdAt" ? String(a.createdAt).localeCompare(String(b.createdAt))
      : String(a[sortKey] ?? "").localeCompare(String(b[sortKey] ?? ""), "it", { sensitivity: "base" });
    return sortDir === "asc" ? r : -r;
  }), [filtered, sortKey, sortDir]);
  function toggleSort(key: SortKey) { if (sortKey === key) setSortDir((d) => d === "asc" ? "desc" : "asc"); else { setSortKey(key); setSortDir(key === "createdAt" || key === "total" ? "desc" : "asc"); } }
  const Th = ({ k, label }: { k: SortKey; label: string }) => <th className="sortable" onClick={() => toggleSort(k)}>{label}{sortKey === k && <MaterialIcon>{sortDir === "asc" ? "arrow_upward" : "arrow_downward"}</MaterialIcon>}</th>;
  function printList() {
    const safe = (v: unknown) => String(v ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#039;" }[c] ?? c));
    const popup = window.open("", "_blank", "width=1000,height=760");
    if (!popup) { alert("Consenti i popup del browser per stampare."); return; }
    const totalSum = visible.reduce((sum, sale) => sum + sale.total, 0);
    const rows = visible.map((sale) => `<tr><td>${safe(dateTime(sale.createdAt))}</td><td>${safe(sale.store)}</td><td>${safe(sale.receiptNo)}</td><td>${safe(sale.customerName)}</td><td class="r">${money(sale.cashAmount)}</td><td class="r">${money(sale.cardAmount)}</td><td class="r">${money(sale.bankAmount)}</td><td class="r">${money(sale.giftAmount)}</td><td class="r"><b>${money(sale.total)}</b></td></tr>`).join("");
    popup.document.write(`<!doctype html><html lang="it"><head><meta charset="utf-8"><title>Registro casse</title><style>body{font:13px Arial,sans-serif;color:#111;margin:26px}header{display:flex;align-items:center;gap:16px;border-bottom:3px solid #12bfe9;padding-bottom:14px;margin-bottom:16px}header img{height:52px}h1{margin:0 0 4px;font-size:21px}header p{color:#555;margin:0;font-size:12px}table{width:100%;border-collapse:collapse}th,td{padding:7px 6px;border-bottom:1px solid #ccc;text-align:left}th{font-size:10px;text-transform:uppercase}td.r,th.r{text-align:right}tfoot td{border-top:2px solid #111;font-weight:bold}@media print{body{margin:10mm}}</style></head><body><header><img src="${window.location.origin}/ms-logo.png" alt="" /><div><h1>Registro casse${ownStore ? ` · ${safe(ownStore)}` : ""}</h1><p>${safe(new Date().toLocaleString("it-IT"))} · ${visible.length} movimenti · ${query ? `filtro: "${safe(query)}"` : "tutti i movimenti"}</p></div></header><table><thead><tr><th>Data</th><th>Sede</th><th>Scontrino</th><th>Cliente</th><th class="r">Contanti</th><th class="r">Carta</th><th class="r">Bonifico</th><th class="r">Buono</th><th class="r">Totale</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="8">Totale (${visible.length} movimenti)</td><td class="r">${money(totalSum)}</td></tr></tfoot></table><script>window.onload=()=>{const done=()=>{window.print();window.close()};const img=document.images[0];if(img&&!img.complete){img.onload=done;img.onerror=done;setTimeout(done,1500)}else done()}<\/script></body></html>`);
    popup.document.close();
  }
  return <section className="screen"><div className="screen-head"><div><p className="eyebrow">RICERCA PER NOMINATIVO</p><h1>Vendite effettuate{ownStore ? ` · ${ownStore}` : ""}</h1><p className="muted inventory-intro">{ownStore ? `Sono visibili esclusivamente le vendite registrate dal negozio ${ownStore}.` : "L’amministratore può consultare le vendite di entrambi i negozi."}</p></div><div className="head-controls"><button className="secondary small" onClick={printList} disabled={!visible.length}><MaterialIcon>print</MaterialIcon> Stampa elenco</button></div></div><div className="metric-grid">{data.user.role === "admin" ? <><Metric label="Netto Viterbo" value={money(data.sales.filter((sale) => sale.store === "Viterbo").reduce((sum, sale) => sum + sale.total, 0))} /><Metric label="Netto Gran Sasso" value={money(data.sales.filter((sale) => sale.store === "Gran Sasso").reduce((sum, sale) => sum + sale.total, 0))} /></> : <Metric label={`Netto ${ownStore}`} value={money(data.sales.reduce((sum, sale) => sum + sale.total, 0))} />}<Metric label="Totale visibile" value={money(data.sales.reduce((sum, sale) => sum + sale.total, 0))} note={`${data.sales.length} movimenti`} /></div><div className="panel"><ClearableInput label="Ricerca per nominativo cliente o numero scontrino" value={query} onChange={setQuery} placeholder="Scrivi nome e cognome…" /><div className="table-wrap"><table><thead><tr><Th k="createdAt" label="Data" /><Th k="store" label="Sede" /><Th k="receiptNo" label="Scontrino" /><Th k="customerName" label="Cliente" /><th>Contanti</th><th>Carta</th><th>Bonifico</th><th>Buono</th><Th k="total" label="Totale" /><th>Stampa</th></tr></thead><tbody>{visible.map((sale) => <tr key={sale.id} className={sale.total < 0 ? "return-row" : ""}><td>{dateTime(sale.createdAt)}</td><td>{sale.store}</td><td>{sale.receiptNo}</td><td>{sale.customerName}</td><td>{money(sale.cashAmount)}</td><td>{money(sale.cardAmount)}</td><td>{money(sale.bankAmount)}</td><td>{money(sale.giftAmount)}</td><td className={sale.total < 0 ? "negative" : ""}><b>{money(sale.total)}</b></td><td><a href={`/api/pdf?type=receipt&id=${sale.id}`}>PDF</a></td></tr>)}</tbody></table></div></div></section>;
}
...
function Settings({ theme, setTheme, isAdmin }: { theme: "dark" | "light"; setTheme: (value: "dark" | "light") => void; isAdmin: boolean }) {
  return <section className="screen"><div className="screen-head"><div><h1>Impostazioni</h1></div></div>
    <div className="panel"><p className="eyebrow">ASPETTO</p><h2>Tema</h2><p className="muted">Scegli tra chiaro e scuro. La preferenza resta salvata per il tuo account.</p><div className="theme-toggle"><button type="button" className={theme === "light" ? "active" : ""} onClick={() => setTheme("light")}><MaterialIcon>light_mode</MaterialIcon> Chiaro</button><button type="button" className={theme === "dark" ? "active" : ""} onClick={() => setTheme("dark")}><MaterialIcon>dark_mode</MaterialIcon> Scuro</button></div></div>
    {isAdmin && <div className="panel"><p className="eyebrow">SICUREZZA ACCESSI</p><h2>Accessi e password</h2><p className="muted">Gli accessi autorizzati sono <strong>admin</strong>, <strong>viterbo</strong> e <strong>gran-sasso</strong>. Le password sono gestite da Supabase Auth.</p><div className="firebase-account-list"><span><b>admin</b><small>Dashboard globale · entrambi i negozi</small></span><span><b>viterbo</b><small>Cassa e dati del negozio Viterbo</small></span><span><b>gran-sasso</b><small>Cassa e dati del negozio Gran Sasso</small></span></div></div>}
  </section>;
}
...
const cashierMenuGroups: { label: string; items: readonly (readonly [string, string, string])[] }[] = [
  { label: "Vendite", items: [["cash", "point_of_sale", "Cassa"], ["register", "list_alt", "Registro casse"]] },
  { label: "Magazzino", items: [["warehouse", "inventory_2", "Magazzino"], ["reorder", "warning", "Da riordinare"], ["transfers", "swap_horiz", "Trasferimenti"]] },
  { label: "Anagrafiche", items: [["customers", "group", "Clienti"], ["reservations", "event_note", "Prenotazioni"], ["repairs", "footprint", "Risuolature"]] },
  { label: "Account", items: [["settings", "settings", "Impostazioni"]] },
];
...
function ScanInfo({ data, product, onAdd }: { data: Bootstrap; product: Product; onAdd: (product: Product) => void }) {
  ...
  const avail = (item: Product) => ({ vt: item.viterboQty - item.viterboReserved, gs: item.granSassoQty - item.granSassoReserved });
  const photoKey = product.photoKey || group.find((item) => item.photoKey)?.photoKey;
  return <div className="scan-card"><div className="scan-hero"><ProductThumb photoKey={photoKey} name={product.name} className="scan-photo" /><div className="scan-hero-info"><p className="eyebrow">{product.brand} · {product.category}</p><h2 className="scan-title">{product.name}</h2><div className="scan-price">{money(product.price)}</div></div></div><div className="scan-variants">{group.map((item) => { const a = avail(item); const out = a.vt <= 0 && a.gs <= 0; const isMatch = item.id === product.id; return <div key={item.id} className={`scan-variant ${isMatch ? "match" : ""} ${out ? "out" : ""}`}><div className="scan-variant-main"><strong>{item.color} · {item.size}{isMatch && <span className="scan-badge">scansionato</span>}</strong><small>{item.sku}</small></div><div className="scan-variant-stock"><span className="chip">VT {a.vt}</span><span className="chip">GS {a.gs}</span></div><button className="primary small" disabled={out} onClick={() => onAdd(item)}>{out ? "Esaurito" : "Aggiungi"}</button></div>; })}</div></div>;
}
...
export default function Gestionale() {
  ...
  const [pendingCashAction, setPendingCashAction] = useState<PendingCashAction | null>(null);
  // Carrello sollevato nello shell: resta pieno cambiando pagina e sopravvive al refresh (sessionStorage).
  const [cart, setCart] = useState<ShellCartItem[]>([]);
  const cartLoaded = useRef(false);
  useEffect(() => { try { const saved = sessionStorage.getItem("gestionale_cart"); if (saved) setCart(JSON.parse(saved) as ShellCartItem[]); } catch { /* storage non disponibile */ } cartLoaded.current = true; }, []);
  useEffect(() => { if (!cartLoaded.current) return; try { sessionStorage.setItem("gestionale_cart", JSON.stringify(cart)); } catch { /* storage non disponibile */ } }, [cart]);
  // Risposte ai trasferimenti già viste dal cassiere (persistono tra le sessioni).
  const [seenTransfers, setSeenTransfers] = useState<Set<number>>(new Set());
  const seenLoaded = useRef(false);
  useEffect(() => { try { const saved = localStorage.getItem("seen_transfer_responses"); if (saved) setSeenTransfers(new Set(JSON.parse(saved) as number[])); } catch { /* storage non disponibile */ } seenLoaded.current = true; }, []);
  useEffect(() => { if (!seenLoaded.current) return; try { localStorage.setItem("seen_transfer_responses", JSON.stringify([...seenTransfers])); } catch { /* storage non disponibile */ } }, [seenTransfers]);
  const [theme, setTheme] = useState<"dark" | "light">("dark");
  useEffect(() => { if (!user) return; try { const saved = localStorage.getItem(`gestionale_theme_${user.username}`); setTheme(saved === "light" ? "light" : "dark"); } catch { /* storage non disponibile */ } }, [user?.username]);
  useEffect(() => { document.documentElement.setAttribute("data-theme", theme); if (!user) return; try { localStorage.setItem(`gestionale_theme_${user.username}`, theme); } catch { /* storage non disponibile */ } }, [theme, user?.username]);
  const newResponses = user && user.role !== "admin" ? (data?.transfers ?? []).filter((transfer) => transfer.status !== "pending" && !seenTransfers.has(transfer.id)) : [];
  const newResponseKey = newResponses.map((transfer) => transfer.id).join(",");
  useEffect(() => { if (page !== "transfers" || !newResponseKey) return; const ids = newResponseKey.split(",").map(Number); setSeenTransfers((prev) => { const next = new Set(prev); ids.forEach((id) => next.add(id)); return next; }); }, [page, newResponseKey]);
  ...
  const pendingTransfers = (data?.transfers ?? []).filter((transfer) => transfer.status === "pending").length;
  const transferBadge = user?.role === "admin" ? pendingTransfers : newResponses.length;
  const navButton = ([id, icon, label]: readonly [string, string, string]) => <button key={id} className={page === id ? "active" : ""} onClick={() => { setPage(id); setMenuOpen(false); }}><MaterialIcon className="nav-icon">{icon}</MaterialIcon>{label}{id === "cash" && cart.length > 0 && <span className="nav-badge" title={`${cart.length} nel carrello`}>{cart.length}</span>}{id === "transfers" && transferBadge > 0 && <span className="nav-badge" title={user?.role === "admin" ? `${transferBadge} richieste in attesa` : `${transferBadge} risposte nuove`}>{transferBadge}</span>}</button>;
  const content: Record<string, ReactNode> = { dashboard: <Dashboard data={data} />, cash: <NewCashRegister data={data} reload={reload} queue={saleQueue} onQueueConsumed={() => setSaleQueue([])} cart={cart} setCart={setCart} pendingAction={pendingCashAction} onPendingConsumed={() => setPendingCashAction(null)} />, customers: <Customers data={data} reload={reload} />, warehouse: <Warehouse data={data} reload={reload} />, inventory: <Inventory data={data} />, reorder: <ReorderList data={data} reload={reload} />, transfers: <Transfers data={data} reload={reload} newResponses={newResponses} />, load: <QuickLoad data={data} reload={reload} />, documents: <Documents data={data} reload={reload} />, register: <Register data={data} />, reports: <CashReports data={data} />, gifts: <GiftSummary data={data} reload={reload} />, reservations: <ReservationSummary data={data} reload={reload} goToCash={(action: PendingCashAction) => { setPendingCashAction(action); setPage("cash"); }} />, repairs: <ReservationSummary data={data} reload={reload} repair goToCash={(action: PendingCashAction) => { setPendingCashAction(action); setPage("cash"); }} />, returns: <Returns data={data} goToCash={(action: PendingCashAction) => { setPendingCashAction(action); setPage("cash"); }} />, analytics: <Analytics data={data} />, storico: <ActivityLog />, fiscal: <FiscalRegisters data={data} reload={reload} />, settings: <Settings theme={theme} setTheme={setTheme} isAdmin={user.role === "admin"} />, services: <Services data={data} reload={reload} /> };
