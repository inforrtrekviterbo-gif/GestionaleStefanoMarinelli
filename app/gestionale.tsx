...
function ReservationSummary({ data, reload, repair = false, goToCash }: { data: Bootstrap; reload: () => Promise<void>; repair?: boolean; goToCash?: (action: PendingCashAction) => void }) {
  ...
  async function consegna(reservation: Reservation) {
    ...
    try {
      ...
      const lines: ShellCartItem[] = items.filter((row) => row.productId).map((row, index) => ({ key: `${keyRes()}-${index}`, productId: row.productId, description: row.description, quantity: row.quantity, unitPrice: row.unitPrice, discountPercent: row.discountPercent ?? 0, itemType: "product", metadata: index === 0 ? { deliverReservationId: reservation.id } : {} }));
      ...
      if (reservation.depositPaid) lines.push({ key: keyRes(), productId: null, description: `Acconto già versato ${reservation.code}`, quantity: 1, unitPrice: -reservation.depositAmount, discountPercent: 0, itemType: "reservation_credit", locked: true, metadata: { reservationId: reservation.id } });
  ...
  return <section className="screen"><div className="screen-head"><div><h1>{repair ? "Risuolature" : "Prenotazioni"}</h1><p className="muted inventory-intro">{repair ? "Riparazioni affidate dai clienti, con acconto e saldo al ritiro." : "Articoli o servizi prenotati dai clienti, con acconto e saldo al ritiro."}</p></div>{!repair && <div className="head-controls"><button className="primary" onClick={() => setShowNew(true)}><MaterialIcon>add</MaterialIcon> Nuova prenotazione</button></div>}</div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<div className="panel"><ClearableInput label="Ricerca codice, prodotto o cliente" value={query} onChange={setQuery} /><div className="table-wrap"><table><thead><tr><th>Codice</th><th>Data</th><th>Sede</th><th>Descrizione</th><th>Articoli</th><th>Cliente</th><th>Acconto</th><th>Saldo</th><th>Stato</th>{!repair && <th>Consegna</th>}<th>PDF EAN</th>{(isAdmin || !repair) && <th>Azioni</th>}</tr></thead><tbody>{visible.map((reservation) => <tr key={reservation.id}><td>{reservation.code}</td><td>{dateTime(reservation.createdAt)}</td><td>{reservation.store}</td><td>{reservation.kind === "repair" ? <><strong>Risuolatura</strong><small>{reservation.itemDescriptions || reservation.description}</small></> : reservation.description}</td><td>{reservation.itemCount || 1}</td><td>{reservation.customerName || "Non associato"}</td><td>{money(reservation.depositAmount)}</td><td><b>{money(reservation.balanceDue)}</b></td><td><span className={`status ${reservation.status === "open" ? "active" : "used"}`}>{reservation.status}</span>{!repair && (reservation.depositPaid ? <span className="status active">acconto incassato</span> : reservation.depositAmount > 0 ? <span className="status warning">acconto da incassare</span> : null)}</td>{!repair && <td>{reservation.expectedDelivery ? new Date(reservation.expectedDelivery).toLocaleDateString("it-IT") : "—"}</td>}<td><a href={`/api/pdf?type=reservation&id=${reservation.id}`}>Scarica</a></td>{(isAdmin || !repair) && <td><div className="table-actions">{!repair && reservation.status === "open" && reservation.depositAmount > 0 && !reservation.depositPaid && <button className="secondary small" onClick={() => incassaAcconto(reservation)}>Incassa acconto</button>}{!repair && reservation.status === "open" && <button className="primary small" onClick={() => void consegna(reservation)}>Consegna</button>}{!repair && reservation.status === "open" && isAdmin && <button className="text-button negative" onClick={() => void cancel(reservation)}>Annulla</button>}{isAdmin && <><button className="secondary small" onClick={() => setEditing(reservation)}>Modifica</button><button className="text-button negative" onClick={() => void remove(reservation)}>Rimuovi</button></>}</div></td>}</tr>)}</tbody></table></div></div>{editing && <Modal title="Modifica prenotazione" onClose={() => setEditing(null)}><ReservationAdminForm reservation={editing} reload={reload} close={() => setEditing(null)} /></Modal>}{!repair && showNew && <Modal title="Nuova prenotazione" wide onClose={() => setShowNew(false)}><ReservationNewForm data={data} reload={reload} onClose={() => setShowNew(false)} /></Modal>}</section>;
}
