...
function ReservationSummary({ data, reload, repair = false, goToCash }: { data: Bootstrap; reload: () => Promise<void>; repair?: boolean; goToCash?: (action: PendingCashAction) => void }) {
  ...
  async function consegna(reservation: Reservation) {
    ...
    try {
      ...
      const lines: ShellCartItem[] = items.filter((row) => row.productId).map((row, index) => ({ key: `${keyRes()}-${index}`, productId: row.productId, description: row.description, quantity: row.quantity, unitPrice: row.unitPrice, discountPercent: 0, itemType: "product", metadata: index === 0 ? { deliverReservationId: reservation.id } : {} }));
