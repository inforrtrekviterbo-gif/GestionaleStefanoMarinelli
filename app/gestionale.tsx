...
function GiftSummary({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  ...
  const isAdmin = data.user.role === "admin";
  const [firstName, setFirstName] = useState(""); const [lastName, setLastName] = useState(""); const [amount, setAmount] = useState(""); const [expiresAt, setExpiresAt] = useState(""); const [store, setStore] = useState<Store>(data.user.store ?? "Viterbo"); const [saving, setSaving] = useState(false); const [issued, setIssued] = useState<{ id: number; code: string } | null>(null);
  ...
  async function issue(event: React.FormEvent) {
    ...
    setSaving(true);
    try { const beneficiary = `${firstName} ${lastName}`.trim(); const result = await post("createGift", { beneficiary, value: Number(amount) || 0, expiresAt, store }); setIssued({ id: result.id, code: result.code }); setFirstName(""); setLastName(""); setAmount(""); setExpiresAt(""); await reload(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); }
    finally { setSaving(false); }
  }
  ...
  return <section className="screen"><div className="screen-head"><div><h1>Buoni regalo</h1><p className="muted inventory-intro">Genera, stampa e cerca i buoni regalo. Sono spendibili in cassa scansionando il codice.</p></div></div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}
    <div className="panel"><div className="panel-title"><div><h2>Genera nuovo buono</h2></div></div><form className="form-grid" onSubmit={issue}><ClearableInput label="Nome" value={firstName} onChange={setFirstName} required /><ClearableInput label="Cognome" value={lastName} onChange={setLastName} required /><ClearableInput label="Importo" type="number" min="0" step="0.01" value={amount} onChange={setAmount} required /><ClearableInput label="Scadenza (facoltativa)" type="date" value={expiresAt} onChange={setExpiresAt} /><label className="field"><span>Negozio emittente</span><select value={store} onChange={(event) => setStore(event.target.value as Store)}><option>Viterbo</option><option>Gran Sasso</option></select></label><div className="form-actions align-end"><button className="primary" disabled={saving}>{saving ? "Creazione…" : "Genera buono"}</button></div></form>{issued && <div className="alert success visual-alert"><MaterialIcon>check_circle</MaterialIcon>Buono <strong>{issued.code}</strong> creato. <a href={`/api/pdf?type=gift&id=${issued.id}`} target="_blank" rel="noreferrer">Stampa PDF</a></div>}</div>
