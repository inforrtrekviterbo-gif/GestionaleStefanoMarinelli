...
function Settings({ reload: _reload }: { reload: () => Promise<void> }) {
  return <section className="screen"><div className="screen-head"><div><h1>Impostazioni</h1></div></div><div className="panel"><p className="eyebrow">SICUREZZA ACCESSI</p><h2>Accessi e password</h2><p className="muted">Gli accessi autorizzati sono <strong>admin</strong>, <strong>viterbo</strong> e <strong>gran-sasso</strong>. Le password sono gestite da Supabase Auth.</p><div className="firebase-account-list"><span><b>admin</b><small>Dashboard globale · entrambi i negozi</small></span><span><b>viterbo</b><small>Cassa e dati del negozio Viterbo</small></span><span><b>gran-sasso</b><small>Cassa e dati del negozio Gran Sasso</small></span></div></div></section>;
}
...
export default function Gestionale() {
  ...
  useEffect(() => { try { const saved = localStorage.getItem("gestionale_theme"); if (saved === "light" || saved === "dark") setTheme(saved); } catch { /* storage non disponibile */ } }, []);
  useEffect(() => { document.documentElement.setAttribute("data-theme", theme); try { localStorage.setItem("gestionale_theme", theme); } catch { /* storage non disponibile */ } }, [theme]);
