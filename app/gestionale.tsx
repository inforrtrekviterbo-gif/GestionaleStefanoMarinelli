...
function ReservationNewForm({ data, reload, onClose, repair = false }: { data: Bootstrap; reload: () => Promise<void>; onClose: () => void; repair?: boolean }) {
  ...
  return <form className="stack" onSubmit={save}>
    {!repair && <div className="customer-type-tabs"><button type="button" className={type === "product" ? "active" : ""} onClick={() => setType("product")}>Prodotti</button><button type="button" className={type === "service" ? "active" : ""} onClick={() => setType("service")}>Servizio</button></div>}
    ...
    : <div className="form-grid"><ClearableInput className="full" label="Descrizione servizio" value={serviceDescription} onChange={setServiceDescription} required /><ClearableInput label="Importo servizio" type="number" min="0" step="0.01" value={serviceAmount} onChange={setServiceAmount} required /></div>}
