...
function Customers({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  const [query, setQuery] = useState(""); const [selected, setSelected] = useState<Customer | null>(null); const [history, setHistory] = useState<{ sales: Sale[]; items: { saleId: number; description: string; quantity: number; lineTotal: number }[] } | null>(null); const [newOpen, setNewOpen] = useState(false); const [editing, setEditing] = useState(false); const [error, setError] = useState("");
  const isAdmin = data.user.role === "admin";
  const visible = useMemo(() => { const q = query.toLocaleLowerCase("it"); return data.customers.filter((item) => !q || `${customerLabel(item)} ${item.phone} ${item.email} ${item.city} ${item.taxCode}`.toLocaleLowerCase("it").includes(q)); }, [data.customers, query]);
  ...
  return <section className="screen"><div className="screen-head"><div><p className="eyebrow">ANAGRAFICHE E STORICO</p><h1>Clienti</h1><p className="muted inventory-intro">Per vendite importanti e garanzia: colleghi lo scontrino al cliente.</p></div><button className="primary" onClick={() => setNewOpen(true)}><MaterialIcon>person_add</MaterialIcon> Nuovo cliente</button></div><div className="panel"><div className="search-hero"><MaterialIcon>search</MaterialIcon><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cerca cliente per nome, telefono, email…" /></div></div><div className="two-columns"><div className="panel list-panel">{visible.length ? visible.map((customer) => <button className={`customer-card ${selected?.id === customer.id ? "selected" : ""}`} key={customer.id} onClick={() => void openCustomer(customer)}><span><strong>{customerLabel(customer)}</strong><small>{customer.phone || customer.email || customer.city || "Nessun recapito"}</small></span><MaterialIcon>chevron_right</MaterialIcon></button>) : (data.customers.length === 0 ? <EmptyGuide icon="group" title="Nessun cliente" text="Aggiungi il primo cliente. Bastano nome e cognome.">{<button className="primary" onClick={() => setNewOpen(true)}><MaterialIcon>person_add</MaterialIcon> Nuovo cliente</button>}</EmptyGuide> : <Empty>Nessun cliente trovato.</Empty>)}</div><div className="panel detail-panel">{!selected || !history ? <Empty>Seleziona un cliente per vedere i dati e le vendite collegate.</Empty> : <><div className="panel-title"><div><p className="eyebrow">CLIENTE</p><h2>{customerLabel(selected)}</h2></div><div className="table-actions">{isAdmin && <><button className="secondary small" onClick={() => setEditing(true)}>Modifica</button><button className="text-button negative" onClick={() => void removeCustomer()}>Rimuovi</button></>}</div></div>{error && <div className="alert danger visual-alert"><MaterialIcon>warning</MaterialIcon>{error}</div>}<div className="customer-contacts">{selected.phone && <span><MaterialIcon>call</MaterialIcon>{selected.phone}</span>}{selected.email && <span><MaterialIcon>mail</MaterialIcon>{selected.email}</span>}{contact && <span><MaterialIcon>location_on</MaterialIcon>{contact}</span>}{selected.taxCode && <span><MaterialIcon>badge</MaterialIcon>{selected.taxCode}</span>}{!selected.phone && !selected.email && !contact && !selected.taxCode && <span className="muted">Nessun recapito registrato.</span>}</div><h3 className="customer-hist-title">Vendite collegate</h3>{history.sales.length ? history.sales.map((sale) => <article className="history-sale" key={sale.id}><div><strong>{sale.receiptNo}</strong><small>{dateTime(sale.createdAt)} · {sale.store}</small></div><b className={sale.total < 0 ? "negative" : ""}>{money(sale.total)}</b><a className="secondary small" href={`/api/pdf?type=receipt&id=${sale.id}`}>Stampa scontrino interno</a><ul>{history.items.filter((item) => item.saleId === sale.id).map((item, index) => <li key={`${sale.id}-${index}`}>{item.quantity} × {item.description} <span>{money(item.lineTotal)}</span></li>)}</ul></article>) : <Empty>Nessuna vendita associata a questo cliente.</Empty>}</>}</div></div>{newOpen && <Modal title="Nuovo cliente" onClose={() => setNewOpen(false)}><CustomerForm store={data.user.store ?? "Viterbo"} onSaved={reload} onClose={() => setNewOpen(false)} /></Modal>}{editing && selected && <Modal title="Modifica cliente" onClose={() => setEditing(false)}><CustomerForm customer={selected} store={selected.createdStore} onSaved={async () => { await reload(); setSelected(null); setHistory(null); }} onClose={() => setEditing(false)} /></Modal>}</section>;
}
