...
function AdminDataManagement({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  const [section, setSection] = useState<"products" | "customers" | "gifts" | "reservations" | "transfers">("products"); const [query, setQuery] = useState(""); const [editing, setEditing] = useState<AdminEdit | null>(null); const [variantSource, setVariantSource] = useState<{ product: Product; mode: "variant" | "duplicate" } | null>(null); const [message, setMessage] = useState(""); const [error, setError] = useState("");
  const normalized = query.trim().toLocaleLowerCase("it");
  async function editTransfer(transfer: Transfer) { setError(""); try { const detail = await readJson(await fetch(`/api/data?view=transfer&id=${transfer.id}`, { cache: "no-store" })); setEditing({ kind: "transfer", record: transfer, items: detail.items as TransferAdminItem[] }); } catch (reason) { setError(reason instanceof Error ? reason.message : "Errore."); } }
...
const adminMenu = [["dashboard", "dashboard", "Riepilogo"], ["warehouse", "inventory_2", "Magazzino"], ["manage", "edit_square", "Modifica e cancella"], ["cash", "point_of_sale", "Cassa"], ["customers", "group", "Clienti"], ["transfers", "swap_horiz", "Trasferimenti"], ["documents", "receipt_long", "Fatturazione"], ["register", "list_alt", "Registro casse"], ["reports", "summarize", "Corrispettivi"], ["gifts", "card_giftcard", "Riepilogo buoni regalo"], ["reservations", "event_note", "Prenotazioni e acconti"], ["analytics", "trending_up", "Analisi vendite"], ["fiscal", "print", "Registratori RT"], ["settings", "settings", "Impostazioni"]] as const;
...
export default function Gestionale() {
  ...
  const content: Record<string, ReactNode> = { dashboard: <Dashboard data={data} />, manage: <AdminDataManagement data={data} reload={reload} />, cash: <NewCashRegister data={data} reload={reload} />, customers: <Customers data={data} reload={reload} />, warehouse: <Warehouse data={data} reload={reload} />, inventory: <Inventory data={data} />, reorder: <ReorderList data={data} />, transfers: <Transfers data={data} reload={reload} />, load: <QuickLoad data={data} reload={reload} />, documents: <Documents data={data} reload={reload} />, register: <Register data={data} />, reports: <CashReports data={data} />, gifts: <GiftSummary data={data} />, reservations: <ReservationSummary data={data} />, analytics: <Analytics data={data} />, fiscal: <FiscalRegisters data={data} reload={reload} />, settings: <Settings reload={reload} /> };
